...
metadata: { "hello-io": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == HelloIo `sessions_spawn`
- **File I/O** == HelloIo `read`/`write`
- **Remote fetch** == HelloIo `web_fetch` (or `exec` with curl when POST is required)
