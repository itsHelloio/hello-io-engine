...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__hello-io_channel_id";
...
type InteractionCallbackConfig = Pick<HelloIoConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "hello-io-mattermost-interactions").update(botToken).digest("hex");
}
