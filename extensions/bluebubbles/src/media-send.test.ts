...
import type { HelloIoConfig, PluginRuntime } from "hello-io/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): HelloIoConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
