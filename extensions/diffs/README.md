...
Read-only diff viewer plugin for **HelloIo** agents.
...
Before:
HelloIo supports plugins.

After:
HelloIo supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/hello-io-diffs`).
