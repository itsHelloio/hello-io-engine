...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-hello-io-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-hello-io-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.hello-ioDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.hello-ioDiffsError = "true";
