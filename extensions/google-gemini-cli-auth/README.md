# Google Gemini CLI Auth (HelloIo plugin)
...
```bash
hello-io plugins enable google-gemini-cli-auth
```
...
```bash
hello-io models auth login --provider google-gemini-cli --set-default
```
...
- `HELLO_IO_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `HELLO_IO_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
