import type { AnyAgentTool, HelloIoPluginApi } from "hello-io/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: HelloIoPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<HelloIoPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
