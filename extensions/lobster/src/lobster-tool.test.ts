...
import type { HelloIoPluginApi, HelloIoPluginToolContext } from "hello-io/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<HelloIoPluginApi> = {}): HelloIoPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<HelloIoPluginToolContext> = {}): HelloIoPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: HelloIoPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
