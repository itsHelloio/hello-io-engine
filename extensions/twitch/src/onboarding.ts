...
import {
  ...
  type WizardPrompter,
} from "hello-io/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: HELLO_IO_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: HelloIoConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var HELLO_IO_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.HELLO_IO_TWITCH_ACCESS_TOKEN?.trim();
