...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["hello-ioPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["hello-io", "pollId"]) ??
    readNestedString(value, ["hello-io", "poll", "id"]) ??
    readNestedString(value, ["data", "hello-ioPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "hello-io", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          hello-ioPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "hello-io poll vote",
          displayText: "Vote recorded",
          value: { hello-ioPollId: pollId, pollId },
        },
      },
    ],
  };
