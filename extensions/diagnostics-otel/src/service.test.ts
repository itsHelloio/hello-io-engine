...
vi.mock("hello-io/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("hello-io/plugin-sdk/diagnostics-otel")>(
    "hello-io/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { HelloIoPluginServiceContext } from "hello-io/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "hello-io/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/hello-io-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): HelloIoPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("hello-io.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("hello-io.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("hello-io.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("hello-io.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("hello-io.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("hello-io.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("hello-io.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("hello-io.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("hello-io.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("hello-io.webhook.processed");
    expect(spanNames).toContain("hello-io.message.processed");
    expect(spanNames).toContain("hello-io.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["hello-io.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("hello-io.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "hello-io.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["hello-io.reason"]).toBe("string");
    expect(String(attrs?.["hello-io.reason"])).not.toContain(
