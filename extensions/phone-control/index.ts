...
import type { HelloIoPluginApi, HelloIoPluginService } from "hello-io/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: HelloIoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: HelloIoPluginApi["config"]): string[] {
...
export default function register(api: HelloIoPluginApi) {
  ...
  const timerService: HelloIoPluginService = {
