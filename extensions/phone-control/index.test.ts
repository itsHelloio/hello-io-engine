...
} from "hello-io/plugin-sdk/phone-control";
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-phone-control-test-"));
    try {
      ...
      let command: HelloIoPluginCommandDefinition | undefined;
