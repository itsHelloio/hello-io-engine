...
`hello-io.invoke --each`) without adding new HelloIo code per workflow.
...
This extension depends on HelloIo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** HelloIo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.hello-io/extensions` as a standalone plugin directory.
