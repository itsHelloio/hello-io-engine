...
function resolveHelloIoRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.HELLO_IO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set HELLO_IO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveHelloIoRoot(), "dist", "extensionAPI.js");
