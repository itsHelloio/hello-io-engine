...
import type { HelloIoPluginApi } from "hello-io/plugin-sdk/device-pair";
import {
...
} from "hello-io/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: HelloIoPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.HELLO_IO_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: HelloIoPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: HelloIoPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the HelloIo iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the HelloIo iOS app:",
            "",
            "```",
            qrAscii,
            "```",
