...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked HELLO_IO_HOME.
  delete process.env.HELLO_IO_HOME;
  process.env.HELLO_IO_STATE_DIR = path.join(base, ".hello-io");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "hello-io-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".hello-io", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
