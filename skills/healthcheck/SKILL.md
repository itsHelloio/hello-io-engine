---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for HelloIo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, HelloIo cron scheduling for periodic checks, or version status checks on a machine running HelloIo (laptop, workstation, Pi, VPS).
---

# HelloIo Host Hardening

## Overview

Assess and harden the host running HelloIo, then align it to a user-defined risk tolerance without breaking access. Use HelloIo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim HelloIo changes the host firewall, SSH, or OS updates; it does not.
...
5. HelloIo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if HelloIo can access sensitive data.
...
### 2) Run HelloIo security audits (read-only)

As part of the default read-only checks, run `hello-io security audit --deep`. Only offer alternatives if the user requests them:

1. `hello-io security audit` (faster, non-probing)
2. `hello-io security audit --json` (structured output)

Offer to apply HelloIo safe defaults (numbered):

1. `hello-io security audit --fix`

Be explicit that `--fix` only tightens HelloIo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check HelloIo version/update status (read-only)

As part of the default read-only checks, run `hello-io update status`.
...
- Credential hygiene notes (location of HelloIo creds, prefer disk encryption)
...
After HelloIo install or first hardening pass, run at least one baseline audit and version check:

- `hello-io security audit`
- `hello-io security audit --deep`
- `hello-io update status`

Ongoing monitoring is recommended. Use the HelloIo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `hello-io cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `hello-io update status`
...
Before creating, `hello-io cron list` and match on exact `name`. If found, `hello-io cron edit <id> ...`.
If not found, `hello-io cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `hello-io update status` (preferred for source checkouts and channels)
2. `npm view hello-io version` (published npm version)

## HelloIo command accuracy

Use only supported commands and flags:

- `hello-io security audit [--deep] [--fix] [--json]`
- `hello-io status` / `hello-io status --deep`
- `hello-io health --json`
- `hello-io update status`
- `hello-io cron add|list|runs|run`

Do not invent CLI flags or imply HelloIo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by HelloIo compaction:
