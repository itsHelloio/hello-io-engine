...
This document describes how the HelloIo ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`hello-io acp` exposes an ACP agent over stdio and forwards prompts to a running
HelloIo Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing HelloIo Gateway
...
- ACP client spawns `hello-io acp` and speaks ACP messages over stdio.
...
`hello-io acp` resolves the Gateway URL and auth from CLI flags or config:
