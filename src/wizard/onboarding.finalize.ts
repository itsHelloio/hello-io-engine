...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: HelloIoConfig;
  nextConfig: HelloIoConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.hello-io.ai/gateway/health",
          "https://docs.hello-io.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.hello-io.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.hello-io/hello-io.json (gateway.auth.token) or HELLO_IO_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("hello-io config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("hello-io doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("hello-io dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control HelloIo."
            : "Copy/paste this URL in a browser on this machine to control HelloIo.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("hello-io dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.hello-io.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.hello-io.ai/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("hello-io configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.hello-io.ai/tools/web"}`,
          "Docs: https://docs.hello-io.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("hello-io configure --section web")}`,
          "",
          "Docs: https://docs.hello-io.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://hello-io.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control HelloIo."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control HelloIo.",
  );
