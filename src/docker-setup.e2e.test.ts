...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "hello-io-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    HELLO_IO_GATEWAY_TOKEN: "test-token",
    HELLO_IO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    HELLO_IO_WORKSPACE_DIR: join(sandbox.rootDir, "hello-io"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      HELLO_IO_EXTRA_MOUNTS: undefined,
      HELLO_IO_HOME_VOLUME: "hello-io-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HELLO_IO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("HELLO_IO_EXTRA_MOUNTS=");
    expect(envFile).toContain("HELLO_IO_HOME_VOLUME=hello-io-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("hello-io-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("hello-io-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg HELLO_IO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm hello-io-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm hello-io-cli config set gateway.mode local");
    expect(log).toContain("run --rm hello-io-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_CONFIG_DIR: configDir,
      HELLO_IO_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when HELLO_IO_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HELLO_IO_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when HELLO_IO_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      ...
      "HELLO_IO_GATEWAY_TOKEN=dotenv-token-123\nHELLO_IO_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HELLO_IO_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "HELLO_IO_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HELLO_IO_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("HELLO_IO_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats HELLO_IO_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HELLO_IO_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg HELLO_IO_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg HELLO_IO_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker hello-io-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        HELLO_IO_SANDBOX: "1",
        HELLO_IO_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("hello-io-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps hello-io-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate hello-io-gateway"));
  ...
  it("rejects injected multiline HELLO_IO_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("HELLO_IO_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid HELLO_IO_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid HELLO_IO_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      HELLO_IO_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("HELLO_IO_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:hello-io-gateway"');
    expect(compose).toContain("depends_on:\n      - hello-io-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/HELLO_IO_GATEWAY_TOKEN: \$\{HELLO_IO_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
});
