...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".hello-io");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.HELLO_IO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$HELLO_IO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via HELLO_IO_STATE_DIR.
