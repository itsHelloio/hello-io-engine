...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop hello-io!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@hello-io_bot", { botUsername: "hello-io_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@hello-io_bot", { botUsername: "hello-io_bot" })).toBe(true);
