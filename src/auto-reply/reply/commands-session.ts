...
import { scheduleGatewaySigusr1Restart, triggerHelloIoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting HelloIo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerHelloIoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting HelloIo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
