...
export function isCommandEnabled(cfg: HelloIoConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: HelloIoConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as HelloIoConfig),
...
export function getCommandDetection(_cfg?: HelloIoConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: HelloIoConfig) {
