...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    hello-ioHome: process.env.HELLO_IO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("HELLO_IO_HOME", snapshot.hello-ioHome);
  restoreKey("HELLO_IO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.HELLO_IO_STATE_DIR = join(home, ".hello-io");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "hello-io-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".hello-io", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): HelloIoConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "hello-io"),
