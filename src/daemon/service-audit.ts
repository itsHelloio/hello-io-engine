...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds HELLO_IO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `hello-io gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service HELLO_IO_GATEWAY_TOKEN does not match gateway.auth.token in hello-io.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.HELLO_IO_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.HELLO_IO_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `hello-io gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
