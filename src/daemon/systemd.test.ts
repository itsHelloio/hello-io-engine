...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false when systemctl is not present", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/hello-io-test-home" } });
  ...
  it("calls systemctl is-enabled when systemctl is present", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "hello-io-gateway.service"]);
      cb(null, "enabled", "");
    });
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/hello-io-test-home" } });
  ...
  it("returns false for the WSL2 Ubuntu 24.04 wrapper-only is-enabled failure", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "hello-io-gateway.service"]);
      const err = new Error(
        "Command failed: systemctl --user is-enabled hello-io-gateway.service",
    ...
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/hello-io-test-home" } }),
    ).rejects.toThrow(
      "systemctl is-enabled unavailable: Command failed: systemctl --user is-enabled hello-io-gateway.service",
    );
  });
  ...
  it("returns false when is-enabled cannot connect to the user bus without machine fallback", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/hello-io-test-home", USER: "", LOGNAME: "" },
      }),
  ...
  it("returns false when both direct and machine-scope is-enabled checks report bus unavailability", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/hello-io-test-home", USER: "debian" },
      }),
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "hello-io-gateway.service"]);
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled hello-io-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when HELLO_IO_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/hello-io-gateway.service",
    },
    {
      name: "uses profile-specific service name when HELLO_IO_PROFILE is set to a custom value",
      env: { HOME: "/home/test", HELLO_IO_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/hello-io-gateway-jbphoenix.service",
    },
    {
      name: "prefers HELLO_IO_SYSTEMD_UNIT over HELLO_IO_PROFILE",
      env: {
        HOME: "/home/test",
        HELLO_IO_PROFILE: "jbphoenix",
        HELLO_IO_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles HELLO_IO_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        HELLO_IO_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from HELLO_IO_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        HELLO_IO_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/hello-io gateway start --name "My Bot"')).toEqual([
      "/usr/bin/hello-io",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('hello-io --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["hello-io", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('hello-io --path "C:\\\\Program Files\\\\HelloIo"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["hello-io", "--path", "C:\\\\Program Files\\\\HelloIo"]);

    expect(
      splitArgsPreservingQuotes('hello-io --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["hello-io", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/hello-io gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads HELLO_IO_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/hello-io-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/hello-io gateway run",
          "EnvironmentFile=%h/.hello-io/.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.hello-io/.env") {
        return "HELLO_IO_GATEWAY_TOKEN=env-file-token\n";
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });

    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.HELLO_IO_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/hello-io-gateway.service")) {
        return [
          ...
          "ExecStart=/usr/bin/hello-io gateway run",
          "EnvironmentFile=%h/.hello-io/.env",
          'Environment="HELLO_IO_GATEWAY_TOKEN=inline-token"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.hello-io/.env") {
        return "HELLO_IO_GATEWAY_TOKEN=env-file-token\n";
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.HELLO_IO_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.HELLO_IO_GATEWAY_TOKEN).toBe("file");
  });
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/hello-io-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/hello-io gateway run",
          "EnvironmentFile=-%h/.hello-io/missing.env",
        ].join("\n");
      }
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.programArguments).toEqual(["/usr/bin/hello-io", "gateway", "run"]);
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/hello-io-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/hello-io gateway run",
          'EnvironmentFile=%h/.hello-io/first.env "%h/.hello-io/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.hello-io/first.env") {
        return "HELLO_IO_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.hello-io/second env.env") {
        return 'HELLO_IO_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      HELLO_IO_GATEWAY_TOKEN: "first-token",
      HELLO_IO_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "HELLO_IO_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "HELLO_IO_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "HELLO_IO_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      HELLO_IO_GATEWAY_TOKEN: "override-token",
      HELLO_IO_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/hello-io-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/hello-io gateway run",
          "EnvironmentFile=%h/.hello-io/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.hello-io/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'HELLO_IO_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "HELLO_IO_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      HELLO_IO_GATEWAY_TOKEN: "quoted token",
      HELLO_IO_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      HELLO_IO_GATEWAY_TOKEN: "file",
      HELLO_IO_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  const assertMachineRestartArgs = (args: string[]) => {
    expect(args).toEqual(["--machine", "debian@", "--user", "restart", "hello-io-gateway.service"]);
  };
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "hello-io-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "hello-io-gateway-work.service"]);
        cb(null, "", "");
      });
    await assertRestartSuccess({ HELLO_IO_PROFILE: "work" });
  });
  ...
  it("keeps direct --user scope when SUDO_USER is root", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "hello-io-gateway.service"]);
        cb(null, "", "");
      });
