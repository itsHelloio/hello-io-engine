...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    HELLO_IO_PROFILE: profile,
    HELLO_IO_GATEWAY_PORT: String(port),
    HELLO_IO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    HELLO_IO_SYSTEMD_UNIT: systemdUnit,
    HELLO_IO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    HELLO_IO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    HELLO_IO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.HELLO_IO_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    HELLO_IO_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    HELLO_IO_STATE_DIR: sharedEnv.stateDir,
    HELLO_IO_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.HELLO_IO_STATE_DIR;
  const configPath = env.HELLO_IO_CONFIG_PATH;
