...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when HELLO_IO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".hello-io", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when HELLO_IO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", HELLO_IO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".hello-io-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers HELLO_IO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        HELLO_IO_PROFILE: "rescue",
        HELLO_IO_STATE_DIR: "C:\\State\\hello-io",
      },
      expected: path.join("C:\\State\\hello-io", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", HELLO_IO_PROFILE: "default" },
      expected: path.join("/home/test", ".hello-io", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem HelloIo Gateway",
          "cd /d C:\\Projects\\hello-io",
          "set NODE_ENV=production",
          "set HELLO_IO_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\hello-io",
          environment: {
            NODE_ENV: "production",
            HELLO_IO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\hello-io\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\hello-io\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\HelloIo Share\\node.exe" "\\\\fileserver\\HelloIo Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\HelloIo Share\\node.exe",
            "\\\\fileserver\\HelloIo Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from HELLO_IO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ HELLO_IO_STATE_DIR: path.join(tmpDir, "custom-state") }),
