...
export function createScopedAccountConfigAccessors<ResolvedAccount>(params: {
  resolveAccount: (params: { cfg: HelloIoConfig; accountId?: string | null }) => ResolvedAccount;
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: HelloIoConfig; accountId?: string | null }) =>
...
export function createScopedChannelConfigBase<
  ResolvedAccount,
  Config extends HelloIoConfig = HelloIoConfig,
>(params: {
  sectionKey: string;
