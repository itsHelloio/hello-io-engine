...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `hello-io-${params.binName}-bin-`));
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-approval-hardening-"));
      const oldPath = process.env.PATH;
  ...
  for (const runtimeCase of mutableOperandCases) {
    it(`captures mutable ${runtimeCase.name} operands in approval plans`, () => {
      withFakeRuntimeBin({
        ...
        run: () => {
          const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-approval-script-plan-"));
  ...
  it("does not snapshot bun package script names", () => {
    withFakeRuntimeBin({
      binName: "bun",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-bun-package-script-"));
        try {
          const prepared = buildSystemRunApprovalPlan({
            command: ["bun", "run", "dev"],
            cwd: tmp,
  ...
  it("does not snapshot deno eval invocations", () => {
    withFakeRuntimeBin({
      binName: "deno",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-deno-eval-"));
        try {
          const prepared = buildSystemRunApprovalPlan({
