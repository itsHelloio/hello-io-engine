...
test("exec sets HELLO_IO_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-hello-io-shell", {
    command: "node -e \"process.stdout.write(process.env.HELLO_IO_SHELL || '')\"",
    pty: true,
  });
