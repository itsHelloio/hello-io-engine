...
import { resolvePreferredHelloIoTmpDir } from "../infra/tmp-hello-io-dir.js";
...
async function withOutsideHardlinkInHelloIoTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside hello-io tmp root",
      media: path.join(os.tmpdir(), "outside-hello-io", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked HelloIo tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked HelloIo tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInHelloIoTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked HelloIo tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInHelloIoTmp(
