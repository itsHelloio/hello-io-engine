...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: HelloIoConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/hello-io" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: HelloIoConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: HelloIoConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: HelloIoConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: HelloIoConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: HelloIoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: HelloIoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: HelloIoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/hello-io");
  });

  it("uses HELLO_IO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "hello-io-home");
    vi.stubEnv("HELLO_IO_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as HelloIoConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".hello-io", "workspace"));
  });

  it("uses HELLO_IO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "hello-io-home");
    vi.stubEnv("HELLO_IO_HOME", home);
    // Clear state dir so it falls back to HELLO_IO_HOME
    vi.stubEnv("HELLO_IO_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as HelloIoConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".hello-io", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/hello-io-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/hello-io-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-agent-scope-"));
