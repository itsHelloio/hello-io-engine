...
function resolveAttachmentLimits(config: HelloIoConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".hello-io", "attachments");
  const relDir = path.posix.join(".hello-io", "attachments", attachmentId);
