...
const GOOGLE_LIVE_SINGLE_KEY = "HELLO_IO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "HELLO_IO_LIVE_ANTHROPIC_KEY",
    listVar: "HELLO_IO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "HELLO_IO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `HELLO_IO_LIVE_${base}_KEY`;
