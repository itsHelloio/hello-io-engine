...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["HELLO_IO_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-subagent-"));
  ...
  it("uses isolated temp state when HELLO_IO_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "hello-io-test-state"));
  });
});
