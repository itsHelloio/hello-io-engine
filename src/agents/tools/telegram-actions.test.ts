...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): HelloIoConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as HelloIoConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): HelloIoConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as HelloIoConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: HelloIoConfig, accountId = "media") {
    await handleTelegramAction(
