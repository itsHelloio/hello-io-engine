...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    hello-io: process.env.HELLO_IO_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.hello-io === undefined) {
      delete process.env.HELLO_IO_GATEWAY_TOKEN;
    } else {
      process.env.HELLO_IO_GATEWAY_TOKEN = envSnapshot.hello-io;
    }
  ...
  it("uses HELLO_IO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.HELLO_IO_GATEWAY_TOKEN = "local-env-token";
