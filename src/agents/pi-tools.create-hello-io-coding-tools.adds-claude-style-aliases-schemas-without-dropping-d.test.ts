...
describe("createHelloIoCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-read-"));
    try {
      ...
      const contents = "Hello from hello-io read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createHelloIoCodingTools({ sandbox });
