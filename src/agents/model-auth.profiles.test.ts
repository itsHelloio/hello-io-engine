...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          HELLO_IO_STATE_DIR: tempDir,
          HELLO_IO_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.HELLO_IO_AGENT_DIR, {
