...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("hello-io-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("hello-io-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("hello-io-sbx-");
