...
const originalBundledDir = process.env.HELLO_IO_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("hello-io-bundled-");
  const workspaceDir = await createTempDir("hello-io-workspace-");
...
afterEach(async () => {
  process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
