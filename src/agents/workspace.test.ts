...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses HELLO_IO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      HELLO_IO_HOME: "/srv/hello-io-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/hello-io-home"), ".hello-io", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".hello-io", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("hello-io-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-workspace-hardlink-"));
    try {
