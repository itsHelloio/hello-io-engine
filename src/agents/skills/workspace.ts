...
function resolveSkillsLimits(config?: HelloIoConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "hello-io-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "hello-io-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveHelloIoMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: HelloIoConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`hello-io skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
