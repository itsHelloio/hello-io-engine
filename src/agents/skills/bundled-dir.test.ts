...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HELLO_IO_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns HELLO_IO_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-bundled-override-"));
    process.env.HELLO_IO_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.HELLO_IO_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "hello-io" }));
