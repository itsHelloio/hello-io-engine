    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "hello-io.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "hello-io.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "hello-io.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("hello-io-");
  const acpxRoot = await tempDirs.make("hello-io-acpx-plugin-");
  const helperRoot = await tempDirs.make("hello-io-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("hello-io-");
  const pluginRoot = await tempDirs.make("hello-io-plugin-");
  const outsideDir = await tempDirs.make("hello-io-outside-");
