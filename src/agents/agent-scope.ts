...
type AgentEntry = NonNullable<NonNullable<HelloIoConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: HelloIoConfig): AgentEntry[] {
...
export function listAgentIds(cfg: HelloIoConfig): string[] {
...
export function resolveDefaultAgentId(cfg: HelloIoConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: HelloIoConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: HelloIoConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: HelloIoConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: HelloIoConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
