...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: HelloIoConfig) {
    return createHelloIoCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<HelloIoConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<HelloIoConfig["agents"]>["list"]>[number]["tools"];
  }): HelloIoConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createHelloIoCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createHelloIoCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createHelloIoCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createHelloIoCodingTools({
      config: cfg,
    ...
    const bobTools = createHelloIoCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createHelloIoCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createHelloIoCodingTools({
      config: cfg,
