...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "hello-io-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.HELLO_IO_BUNDLED_SKILLS_DIR = bundledDir;
