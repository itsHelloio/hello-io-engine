...
`;
    ...
    expect(parsed.hello-io.emoji).toBe("💾");
    expect(parsed.hello-io.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.hello-io.emoji).toBe("📝");
    expect(parsed.hello-io.events).toEqual(["command"]);
    expect(parsed.hello-io.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.hello-io.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"hello-io": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"hello-io": {"events": ["test"]}}');
  });
...
describe("resolveHelloIoMetadata", () => {
  it("extracts hello-io metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveHelloIoMetadata(frontmatter);
  ...
  it("returns undefined when hello-io key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        hello-io: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with HelloIo" },
            { id: "npm", kind: "npm", package: "@hello-io/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@hello-io/hook");
  });
    ...
    "hello-io":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with HelloIo" }],
      },
  }
---
...
`;
    ...
    const hello-io = resolveHelloIoMetadata(frontmatter);
    expect(hello-io).toBeDefined();
    expect(hello-io?.emoji).toBe("💾");
    expect(hello-io?.events).toEqual(["command:new", "command:reset"]);
    expect(hello-io?.requires?.config).toEqual(["workspace.dir"]);
    expect(hello-io?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const hello-io = resolveHelloIoMetadata(frontmatter);
    expect(hello-io?.emoji).toBe("disk");
    expect(hello-io?.events).toEqual(["command:new"]);
  });
});
