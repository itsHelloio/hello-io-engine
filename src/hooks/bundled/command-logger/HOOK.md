...
homepage: https://docs.hello-io.ai/automation/hooks#command-logger
metadata:
  {
    "hello-io":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.hello-io/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.hello-io/logs/commands.log ~/.hello-io/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/hello-io`:
   ```
   /home/username/.hello-io/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
