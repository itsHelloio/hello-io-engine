...
This directory contains hooks that ship with HelloIo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.hello-io/hello-io.json`:
...
2. Restart gateway: `pkill -9 -f 'hello-io.*gateway' && pnpm hello-io gateway`
3. Enable the hook: `hello-io hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.hello-io.ai/automation/hooks
