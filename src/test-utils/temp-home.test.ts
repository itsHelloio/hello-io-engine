...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.HELLO_IO_STATE_DIR;

    const tempHome = await createTempHomeEnv("hello-io-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.HELLO_IO_STATE_DIR).toBe(path.join(tempHome.home, ".hello-io"));
    ...
    expect(process.env.HELLO_IO_STATE_DIR).toBe(previousStateDir);
