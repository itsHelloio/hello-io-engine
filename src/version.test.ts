...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "hello-io", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "hello-io", version: "2.3.4" });
  ...
  it("ignores non-hello-io package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers HELLO_IO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        HELLO_IO_VERSION: "9.9.9",
        HELLO_IO_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        HELLO_IO_VERSION: "   ",
        HELLO_IO_SERVICE_VERSION: "  2.0.0  ",
