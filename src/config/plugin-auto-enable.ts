...
function isWhatsAppConfigured(cfg: HelloIoConfig): boolean {
...
function isGenericChannelConfigured(cfg: HelloIoConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: HelloIoConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: HelloIoConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: HelloIoConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: HelloIoConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: HelloIoConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: HelloIoConfig, pluginId: string): HelloIoConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
