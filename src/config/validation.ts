...
import type { HelloIoConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: HelloIoConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: HelloIoConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: HelloIoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = HelloIoSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as HelloIoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as HelloIoConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as HelloIoConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as HelloIoConfig,
  };
}
