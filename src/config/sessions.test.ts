...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ HELLO_IO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-hello-io",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-hello-io#general");
  });
  ...
  it("derives session transcripts dir from HELLO_IO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { HELLO_IO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when HELLO_IO_STATE_DIR differs from stored paths", () => {
