...
import type { ConfigFileSnapshot } from "./types.hello-io.js";
import { HelloIoSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(HelloIoSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.hello-io/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/hello-io/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/hello-io/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/hello-io/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/hello-io/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = HelloIoSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "HelloIoConfig";
    const hints = mainSchemaHints;
