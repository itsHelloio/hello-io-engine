...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("hello-io-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "hello-io-sandbox-browser-global" },
      agentBrowser: { network: "hello-io-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("hello-io-sandbox-browser-agent");
  });
