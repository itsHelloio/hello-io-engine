...
function collectConfigEnvVarsByTarget(cfg?: HelloIoConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: HelloIoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: HelloIoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: HelloIoConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.HELLO_IO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
