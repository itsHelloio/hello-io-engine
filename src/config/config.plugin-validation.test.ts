...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "hello-io.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    HELLO_IO_PLUGIN_MANIFEST_CACHE_MS: process.env.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-config-plugin-validation-"));
    ...
    process.env.HELLO_IO_STATE_DIR = path.join(suiteHome, ".hello-io");
    process.env.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.HELLO_IO_STATE_DIR === undefined) {
      delete process.env.HELLO_IO_STATE_DIR;
    } else {
      process.env.HELLO_IO_STATE_DIR = envSnapshot.HELLO_IO_STATE_DIR;
    }
    if (envSnapshot.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.HELLO_IO_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
