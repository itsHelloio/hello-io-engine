...
import { buildWebSearchProviderConfig, withTempHome, writeHelloIoConfig } from "./test-helpers.js";
import { HelloIoSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = HelloIoSchema.safeParse({
      $schema: "https://hello-io.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://hello-io.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = HelloIoSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = HelloIoSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeHelloIoConfig(home, {
        gateway: { bind: "${HELLO_IO_BIND}" },
      });

      const prev = process.env.HELLO_IO_BIND;
      process.env.HELLO_IO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.HELLO_IO_BIND;
        } else {
          process.env.HELLO_IO_BIND = prev;
        }
      }
    });
  });
