...
describe("shared/frontmatter", () => {
  ...
  test("resolveHelloIoManifestBlock parses JSON5 metadata and picks hello-io block", () => {
    const frontmatter = {
      metadata: "{ hello-io: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveHelloIoManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveHelloIoManifestBlock returns undefined for invalid input", () => {
    expect(resolveHelloIoManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveHelloIoManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveHelloIoManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
