...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-install-sh-"));
  const cliPath = path.join(root, "hello-io");
...
HELLO_IO_BIN="$FAKE_HELLO_IO_BIN"
resolve_hello-io_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_HELLO_IO_BIN: cliPath,
        HELLO_IO_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("HelloIo 2026.3.9 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("HelloIo dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("HelloIo dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-install-stdin-"));
