...
const STEP_LABELS: Record<string, string> = {
  ...
  "hello-io doctor entry": "Checking doctor entrypoint",
  "hello-io doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g hello-io@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g hello-io@latest --omit=optional");
  }
