    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hello-io nodes status", "List known nodes with live status."],
          ["hello-io nodes pairing pending", "Show pending node pairing requests."],
          ['hello-io nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["hello-io nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.hello-io.ai/cli/nodes")}\n`,
    );
