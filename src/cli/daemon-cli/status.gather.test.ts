...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    HELLO_IO_STATE_DIR: "/tmp/hello-io-daemon",
    HELLO_IO_CONFIG_PATH: "/tmp/hello-io-daemon/hello-io.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.HELLO_IO_STATE_DIR ?? "/tmp/hello-io-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.HELLO_IO_CONFIG_PATH ?? `${stateDir}/hello-io.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/hello-io-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.HELLO_IO_STATE_DIR = "/tmp/hello-io-cli";
    process.env.HELLO_IO_CONFIG_PATH = "/tmp/hello-io-cli/hello-io.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.HELLO_IO_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
