...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-bundled-skills-test-"));
    process.env.HELLO_IO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
