...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update HelloIo and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["hello-io update", "Update a source checkout (git)"],
        ["hello-io update --channel beta", "Switch to beta channel (git + npm)"],
        ["hello-io update --channel dev", "Switch to dev channel (git + npm)"],
        ["hello-io update --tag beta", "One-off update to a dist-tag or version"],
        ["hello-io update --dry-run", "Preview actions without changing anything"],
        ["hello-io update --no-restart", "Update without restarting the service"],
        ["hello-io update --json", "Output result as JSON"],
        ["hello-io update --yes", "Non-interactive (accept downgrade prompts)"],
        ["hello-io update wizard", "Interactive update wizard"],
        ["hello-io --update", "Shorthand for hello-io update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run hello-io update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.hello-io.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hello-io update status", "Show channel + version status."],
          ["hello-io update status --json", "JSON output."],
          ["hello-io update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.hello-io.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
