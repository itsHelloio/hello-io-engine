    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "hello-io node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["hello-io node status", "Check node host service status."],
          ["hello-io node install", "Install the node host service."],
          ["hello-io node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.hello-io.ai/cli/node")}\n`,
    );
