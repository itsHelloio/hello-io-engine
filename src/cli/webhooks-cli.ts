...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.hello-io.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + HelloIo hooks")
    ...
    .option("--hook-url <url>", "HelloIo hook URL")
    .option("--hook-token <token>", "HelloIo hook token")
