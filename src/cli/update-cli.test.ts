...
import type { HelloIoConfig, ConfigFileSnapshot } from "../config/types.hello-io.js";
...
vi.mock("../infra/hello-io-root.js", () => ({
  resolveHelloIoPackageRoot: vi.fn(),
}));
...
const { resolveHelloIoPackageRoot } = await import("../infra/hello-io-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/hello-io-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as HelloIoConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/hello-io-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveHelloIoPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("hello-io-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveHelloIoPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("hello-io");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/hello-io-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "hello-io-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("HelloIo update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as HelloIoConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveHelloIoPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("hello-io-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ HELLO_IO_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.HELLO_IO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("hello-io-update-wizard");
    await withEnvAsync({ HELLO_IO_GIT_DIR: tempDir }, async () => {
      setTty(true);
