...
import { discoverHelloIoPlugins } from "../../plugins/discovery.js";
import type { HelloIoPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, HelloIoPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["HELLO_IO_PLUGIN_CATALOG_PATHS", "HELLO_IO_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<HelloIoPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: HelloIoPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: HelloIoPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverHelloIoPlugins({ workspaceDir: options.workspaceDir });
