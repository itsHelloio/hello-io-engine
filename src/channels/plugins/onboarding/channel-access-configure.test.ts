...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: HelloIoConfig, policy: ChannelAccessPolicy) => HelloIoConfig;
  resolveAllowlist: (params: { cfg: HelloIoConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: HelloIoConfig; resolved: TResolved }) => HelloIoConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: HelloIoConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: HelloIoConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: HelloIoConfig, policy: ChannelAccessPolicy): HelloIoConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: HelloIoConfig, policy: ChannelAccessPolicy): HelloIoConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: HelloIoConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: HelloIoConfig; resolved: string[] }) => {
      calls.push("apply");
