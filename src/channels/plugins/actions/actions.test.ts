...
function telegramCfg(): HelloIoConfig {
  return { channels: { telegram: { botToken: "tok" } } } as HelloIoConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: HelloIoConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as HelloIoConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as HelloIoConfig;
...
function createSignalAccountOverrideCfg(): HelloIoConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as HelloIoConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as HelloIoConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as HelloIoConfig,
        accountId: undefined,
        params: {
