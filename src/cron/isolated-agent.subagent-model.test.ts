...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "hello-io-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".hello-io", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<HelloIoConfig> = {},
): HelloIoConfig {
  const base: HelloIoConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<HelloIoConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<HelloIoConfig>,
