...
const { makeStorePath } = createCronStoreHarness({ prefix: "hello-io-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses HELLO_IO_HOME for tilde expansion", () => {
    vi.stubEnv("HELLO_IO_HOME", "/srv/hello-io-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/hello-io-home", "cron", "jobs.json"));
  });
});
