...
export function resolveHooksConfig(cfg: HelloIoConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: HelloIoConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-hello-io-token"] === "string"
      ? req.headers["x-hello-io-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
