  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set HELLO_IO_GATEWAY_TOKEN/HELLO_IO_GATEWAY_PASSWORD, pass explicit --token/--password,",
 ...
 * This prevents literal placeholder strings like `${HELLO_IO_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.HELLO_IO_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.HELLO_IO_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.HELLO_IO_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
