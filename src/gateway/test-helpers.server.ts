...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "HELLO_IO_SKIP_BROWSER_CONTROL_SERVER",
  "HELLO_IO_SKIP_GMAIL_WATCHER",
  "HELLO_IO_SKIP_CANVAS_HOST",
  "HELLO_IO_BUNDLED_PLUGINS_DIR",
  "HELLO_IO_SKIP_CHANNELS",
  "HELLO_IO_SKIP_PROVIDERS",
  "HELLO_IO_SKIP_CRON",
  "HELLO_IO_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.HELLO_IO_STATE_DIR = path.join(tempHome, ".hello-io");
  delete process.env.HELLO_IO_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.HELLO_IO_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.HELLO_IO_SKIP_GMAIL_WATCHER = "1";
  process.env.HELLO_IO_SKIP_CANVAS_HOST = "1";
  process.env.HELLO_IO_SKIP_CHANNELS = "1";
  process.env.HELLO_IO_SKIP_PROVIDERS = "1";
  process.env.HELLO_IO_SKIP_CRON = "1";
  process.env.HELLO_IO_TEST_MINIMAL_GATEWAY = "1";
  process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "hello-io-test-no-bundled-extensions")
    : "hello-io-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".hello-io-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".hello-io-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__hello-ioTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__hello-ioTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["HELLO_IO_GATEWAY_TOKEN"]);
  const prev = process.env.HELLO_IO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.HELLO_IO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.HELLO_IO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
