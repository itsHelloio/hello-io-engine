...
const { createHelloIoTools } = await import("../agents/hello-io-tools.js");
...
type SessionSendTool = ReturnType<typeof createHelloIoTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createHelloIoTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["HELLO_IO_GATEWAY_PORT", "HELLO_IO_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.HELLO_IO_GATEWAY_PORT = String(gatewayPort);
  process.env.HELLO_IO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("HELLO_IO_CONFIG_PATH missing in gateway test environment");
      }
