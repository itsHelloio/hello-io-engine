...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.HELLO_IO_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.HELLO_IO_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set HELLO_IO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: hello-io doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__HELLO_IO_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
