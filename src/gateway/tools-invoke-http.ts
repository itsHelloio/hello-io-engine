...
import { createHelloIoTools } from "../agents/hello-io-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-hello-io-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-hello-io-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-hello-io-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-hello-io-thread-id")?.trim() || undefined;
  ...
  const allTools = createHelloIoTools({
