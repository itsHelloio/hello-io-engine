...
let lastCreateHelloIoToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/hello-io-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateHelloIoToolsContext?.agentTo,
          agentThreadId: lastCreateHelloIoToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createHelloIoTools: (ctx: Record<string, unknown>) => {
      lastCreateHelloIoToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateHelloIoToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateHelloIoToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-hello-io-message-to": "channel:24514",
        "x-hello-io-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
