...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.hello-ioCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.hello-ioCanvasA2UIAction &&
        typeof window.hello-ioCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.hello-ioSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("hello-io:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android HelloIo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.hello-ioSendUserAction === "function"
        ? window.hello-ioSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.HELLO_IO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.HELLO_IO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>HelloIo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
