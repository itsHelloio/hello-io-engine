...
describe("canvas host state dir defaults", () => {
  it("uses HELLO_IO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("hello-io-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("HelloIo Canvas");
      } finally {
        await handler.close();
      }
    });
