...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("HELLO_IO_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  it("reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.HELLO_IO_STATE_DIR = previousStateDir;
      }
      await fs.rm(stateDir, { recursive: true, force: true });
    }
  });
