...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set HELLO_IO_GATEWAY_TOKEN/HELLO_IO_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "hello-io-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.HELLO_IO_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.HELLO_IO_GATEWAY_PASSWORD);
