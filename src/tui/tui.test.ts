...
describe("resolveInitialTuiAgentId", () => {
  const cfg: HelloIoConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/hello-io" },
        { id: "ops", workspace: "/tmp/hello-io/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/hello-io/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run hello-io devices list");
    expect(state.pairingHint).toContain("hello-io devices list");
  });
