...
let originalHelloIoLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.HELLO_IO_TEST_FILE_LOG;
  originalHelloIoLogLevel = process.env.HELLO_IO_LOG_LEVEL;
  delete process.env.HELLO_IO_TEST_FILE_LOG;
  delete process.env.HELLO_IO_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.HELLO_IO_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalHelloIoLogLevel === undefined) {
    delete process.env.HELLO_IO_LOG_LEVEL;
  } else {
    process.env.HELLO_IO_LOG_LEVEL = originalHelloIoLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
