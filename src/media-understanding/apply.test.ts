...
const TEMP_MEDIA_PREFIX = "hello-io-media-";
...
function createGroqAudioConfig(): HelloIoConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): HelloIoConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): HelloIoConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: HelloIoConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredHelloIoTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        HELLO_IO_AGENT_DIR: isolatedAgentDir,
