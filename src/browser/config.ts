import type { BrowserConfig, BrowserProfileConfig, HelloIoConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_HELLO_IO_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_HELLO_IO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "hello-io" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an HelloIo browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for hello-io/hello-io CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: HelloIoConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_HELLO_IO_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME]
        ? DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "hello-io";
