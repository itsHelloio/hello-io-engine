...
const { stopHelloIoChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopHelloIoChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopHelloIoChrome: stopHelloIoChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "hello-io", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopHelloIoChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["hello-io", "chrome"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      hello-io: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.hello-io).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopHelloIoChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("hello-io browser stop failed: Error: oops");
  });
});
