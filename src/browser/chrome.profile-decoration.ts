...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".hello-io-profile-decorated");
}
 ...
 */
export function decorateHelloIoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_HELLO_IO_BROWSER_COLOR).toUpperCase();
