...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-hello-io-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    try {
      ...
      expect(headers["x-hello-io-relay-token"]).toBeTruthy();
      expect(headers["x-hello-io-relay-token"]).not.toBe("test-gateway-token");
    } finally {
      await stopChromeExtensionRelayServer({ cdpUrl }).catch(() => {});
      if (prev === undefined) {
        delete process.env.HELLO_IO_GATEWAY_TOKEN;
      } else {
        process.env.HELLO_IO_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!hello-io) {
      throw new Error("expected hello-io profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...hello-io, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
