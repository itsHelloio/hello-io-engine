...
const RELAY_AUTH_HEADER = "x-hello-io-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/HelloIo-Extension-Relay",
            revision: "0",
            userAgent: "HelloIo-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "HelloIo/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedHelloIoRelay({
