  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          hello-io: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.HELLO_IO_GATEWAY_PORT;
    process.env.HELLO_IO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.HELLO_IO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.HELLO_IO_GATEWAY_PASSWORD;
