...
import {
  probeAuthenticatedHelloIoRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedHelloIoRelay({
    baseUrl,
    relayAuthHeader: "x-hello-io-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated hello-io relay probe responses", async () => {
