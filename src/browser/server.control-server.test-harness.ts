...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.HELLO_IO_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/hello-io" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "hello-io",
        profiles: {
          hello-io: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchHelloIoChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.HELLO_IO_GATEWAY_PORT;
  process.env.HELLO_IO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.HELLO_IO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.HELLO_IO_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.HELLO_IO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.HELLO_IO_GATEWAY_PASSWORD;
  } else {
    process.env.HELLO_IO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
