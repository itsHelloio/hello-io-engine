...
vi.mock("./chrome.js", () => ({
  resolveHelloIoUserDataDir: vi.fn(() => "/tmp/hello-io-test/hello-io/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveHelloIoUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "hello-io-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveHelloIoUserDataDir).mockReturnValue(userDataDir);
