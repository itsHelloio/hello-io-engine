...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredHelloIoTmpDir: vi.fn(() => "/tmp/hello-io"),
}));
vi.mock("../infra/tmp-hello-io-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredHelloIoTmpDir.mockReturnValue("/tmp/hello-io");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".hello-io-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredHelloIoTmpDir.mockReturnValue("/tmp/hello-io-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "hello-io-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "hello-io-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredHelloIoTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "hello-io-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "hello-io-preferred", "downloads")}${path.sep}`),
    );
  });
