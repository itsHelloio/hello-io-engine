...
export function resolveHelloIoUserDataDir(profileName = DEFAULT_HELLO_IO_BROWSER_PROFILE_NAME) {
...
export async function launchHelloIoChrome(
  ...
  const userDataDir = resolveHelloIoUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_HELLO_IO_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateHelloIoProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 hello-io browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`hello-io browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`hello-io browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 hello-io browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopHelloIoChrome(
