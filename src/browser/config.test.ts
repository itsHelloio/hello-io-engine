...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("hello-io");
    expect(profile?.driver).toBe("hello-io");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const hello-io = resolveProfile(resolved, "hello-io");
    expect(hello-io?.driver).toBe("hello-io");
    expect(hello-io?.cdpPort).toBe(18800);
    expect(hello-io?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from HELLO_IO_GATEWAY_PORT when unset", () => {
    withEnv({ HELLO_IO_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const hello-io = resolveProfile(resolved, "hello-io");
      expect(hello-io?.cdpPort).toBe(19012);
      expect(hello-io?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ HELLO_IO_GATEWAY_PORT: undefined }, () => {
      ...
      const hello-io = resolveProfile(resolved, "hello-io");
      expect(hello-io?.cdpPort).toBe(19022);
      expect(hello-io?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(hello-io?.cdpPort).toBe(19000);
    expect(hello-io?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "hello-io");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        hello-io: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("hello-io");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to hello-io profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps hello-io default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("hello-io");
    ...
    it("keeps hello-io default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("hello-io");
    ...
    it("keeps hello-io default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
