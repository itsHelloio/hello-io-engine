...
function localHelloIoProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "hello-io",
...
function createLocalHelloIoResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localHelloIoProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveHelloIoUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-reset-"));
    const profileDir = path.join(tempRoot, "hello-io");
    ...
    const ops = createLocalHelloIoResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveHelloIoUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-reset-no-own-"));
    const profileDir = path.join(tempRoot, "hello-io");
    fs.mkdirSync(profileDir, { recursive: true });
