...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        HELLO_IO_BASE_ENV: "base",
        HELLO_IO_TO_REMOVE: undefined,
      },
      env: {
        HELLO_IO_TEST_ENV: "ok",
      },
    });

    expect(resolved.HELLO_IO_BASE_ENV).toBe("base");
    expect(resolved.HELLO_IO_TEST_ENV).toBe("ok");
    expect(resolved.HELLO_IO_TO_REMOVE).toBeUndefined();
    expect(resolved.HELLO_IO_CLI).toBe(HELLO_IO_CLI_ENV_VALUE);
  });
