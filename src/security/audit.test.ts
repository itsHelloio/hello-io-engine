...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: HelloIoConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: HelloIoConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: HelloIoConfig) => string[];
  isConfigured?: (account: unknown, cfg: HelloIoConfig) => boolean;
  isEnabled?: (account: unknown, cfg: HelloIoConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "hello-io.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ HELLO_IO_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        hello-io: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.HELLO_IO_GATEWAY_TOKEN;
    const prevPassword = process.env.HELLO_IO_GATEWAY_PASSWORD;
    delete process.env.HELLO_IO_GATEWAY_TOKEN;
    delete process.env.HELLO_IO_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.HELLO_IO_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.HELLO_IO_GATEWAY_PASSWORD;
      } else {
        process.env.HELLO_IO_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/hello-io-safe-bins"];
    const cfg: HelloIoConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("hello-io-sbx-browser-old\nhello-io-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "hello-io-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "hello-io-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("hello-io-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("hello-io-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "hello-io-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "hello-io-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-hello-io.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: HelloIoConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: HelloIoConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): HelloIoConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: HelloIoConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: HelloIoConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: HelloIoConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.hello-io/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: HelloIoConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.HELLO_IO_GATEWAY_TOKEN;
    process.env.HELLO_IO_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: HelloIoConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<HelloIoConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: HelloIoConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.hello-io",
      configPath: "/Users/test/Dropbox/.hello-io/hello-io.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: HelloIoConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "hello-io.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "hello-io.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: HelloIoConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@hello-io/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@hello-io/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@hello-io/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@hello-io/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        hello-io: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.HELLO_IO_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.HELLO_IO_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
