...
import type { ConfigFileSnapshot, HelloIoConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: HelloIoConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your HelloIo state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your HelloIo state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.HELLO_IO_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.HELLO_IO_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("hello-io security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.HELLO_IO_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: HelloIoConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: HelloIoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: HelloIoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("hello-io status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("hello-io security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set HELLO_IO_GATEWAY_TOKEN/HELLO_IO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("hello-io security audit --deep")}".`,
    });
  }
