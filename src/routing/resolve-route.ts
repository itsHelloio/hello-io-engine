...
function listAgents(cfg: HelloIoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: HelloIoConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<HelloIoConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: HelloIoConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: HelloIoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: HelloIoConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<HelloIoConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  HelloIoConfig,
  {
    bindingsRef: HelloIoConfig["bindings"];
    agentsRef: HelloIoConfig["agents"];
    sessionRef: HelloIoConfig["session"];
...
function resolveRouteCacheForConfig(cfg: HelloIoConfig): Map<string, ResolvedAgentRoute> {
