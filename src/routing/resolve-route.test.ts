...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: HelloIoConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: HelloIoConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/hello-io-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<HelloIoConfig["bindings"]>[number];
