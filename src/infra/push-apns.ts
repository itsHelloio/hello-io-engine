...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.HELLO_IO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.HELLO_IO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set HELLO_IO_APNS_TEAM_ID and HELLO_IO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.HELLO_IO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.HELLO_IO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.HELLO_IO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set HELLO_IO_APNS_PRIVATE_KEY_P8 or HELLO_IO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading HELLO_IO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createHelloIoPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    hello-io: createHelloIoPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
