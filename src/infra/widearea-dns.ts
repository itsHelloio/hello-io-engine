...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.HELLO_IO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*hello-io-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "hello-io";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "hello-io");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "hello-io-gw");
  ...
  records.push(`_hello-io-gw._tcp IN PTR ${instanceLabel}._hello-io-gw._tcp`);
  records.push(`${instanceLabel}._hello-io-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._hello-io-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; hello-io-content-hash: ${contentHash}\n${contentBody}`;
}
