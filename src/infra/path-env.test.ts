...
let ensureHelloIoCliOnPath: typeof import("./path-env.js").ensureHelloIoCliOnPath;

describe("ensureHelloIoCliOnPath", () => {
  const envKeys = [
    "PATH",
    "HELLO_IO_PATH_BOOTSTRAPPED",
    "HELLO_IO_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureHelloIoCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling hello-io exists", () => {
    const tmp = abs("/tmp/hello-io-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "hello-io");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.HELLO_IO_PATH_BOOTSTRAPPED;

    ensureHelloIoCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.HELLO_IO_PATH_BOOTSTRAPPED = "1";
    ensureHelloIoCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/hello-io-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "hello-io");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/hello-io-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "hello-io");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "hello-io");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/hello-io-path/case-linuxbrew");
