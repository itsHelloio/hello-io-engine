...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: HelloIoConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: HelloIoConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: HelloIoConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: HelloIoConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: HelloIoConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: HelloIoConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: HelloIoConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
