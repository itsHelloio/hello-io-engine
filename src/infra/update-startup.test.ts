...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["HELLO_IO_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.HELLO_IO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveHelloIoPackageRoot } = await import("./hello-io-root.js"));
    ...
    vi.mocked(resolveHelloIoPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveHelloIoPackageRoot).mockResolvedValue("/opt/hello-io");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/hello-io/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
