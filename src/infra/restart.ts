...
export function triggerHelloIoRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.HELLO_IO_SYSTEMD_UNIT,
      process.env.HELLO_IO_PROFILE,
    );
  ...
  const label =
    process.env.HELLO_IO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.HELLO_IO_PROFILE);
