...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __hello-ioDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__hello-ioDiagnosticEventsState) {
    globalStore.__hello-ioDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__hello-ioDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: HelloIoConfig): boolean {
