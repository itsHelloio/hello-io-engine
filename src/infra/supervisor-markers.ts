const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "HELLO_IO_LAUNCHD_LABEL"],
  systemd: ["HELLO_IO_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["HELLO_IO_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "HELLO_IO_SERVICE_MARKER",
  "HELLO_IO_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.HELLO_IO_SERVICE_MARKER?.trim();
    const serviceKind = env.HELLO_IO_SERVICE_KIND?.trim();
