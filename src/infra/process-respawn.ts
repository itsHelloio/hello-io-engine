...
import { triggerHelloIoRestart } from "./restart.js";
 ...
 * - HELLO_IO_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.HELLO_IO_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    // See: https://github.com/hello-io/hello-io/issues/39760
    if (supervisor === "schtasks") {
      const restart = triggerHelloIoRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
