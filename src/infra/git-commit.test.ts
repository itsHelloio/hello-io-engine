...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `hello-io-${label}-`));
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the hello-io package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "hello-io");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "hello-io", version: "2026.3.9" }),
      "utf-8",
    );
