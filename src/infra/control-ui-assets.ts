...
import { resolveHelloIoPackageRoot, resolveHelloIoPackageRootSync } from "./hello-io-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveHelloIoPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "hello-io" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // hello-io.mjs or dist/<bundle>.js
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/hello-io -> .../dist/index.js)
