...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: HelloIoConfig | null = null;
  ...
  get(key: string, cfg: HelloIoConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: HelloIoConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: HelloIoConfig): void {
