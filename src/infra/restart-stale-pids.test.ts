...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses hello-io-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "hello-io-gateway" },
          { pid: process.pid, cmd: "hello-io-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'hello-io'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple hello-io pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "hello-io-gateway" },
          { pid: pid2, cmd: "hello-io-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-hello-io pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks hello-io processes — non-hello-io listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "hello-io-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty hello-io stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty hello-io stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + hello-io pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "hello-io-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'hello-io'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("hello-io")
      // inside the mid-loop flush: a non-hello-io cmd between two entries must not
      // be pushed, but the following hello-io entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-hello-io entry first, then hello-io entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid hello-io entry must still be found.
      const stdout = `p0\nchello-io-gateway\np${stalePid}\nchello-io-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero hello-io pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-hello-io stdout (line 145)", () => {
    it("treats status 1 + non-hello-io stdout as port-free (not an hello-io process)", () => {
      // status 1 + non-empty stdout where no hello-io pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill hello-io pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-hello-io output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no hello-io pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
