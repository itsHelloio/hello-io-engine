...
function candidateBinDirs(opts: EnsureHelloIoPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `hello-io` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "hello-io");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.HELLO_IO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "hello-io"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `hello-io` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureHelloIoCliOnPath(opts: EnsureHelloIoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.HELLO_IO_PATH_BOOTSTRAPPED)) {
    return;
  }
