...
import { POSIX_HELLO_IO_TMP_DIR, resolvePreferredHelloIoTmpDir } from "./tmp-hello-io-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredHelloIoTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `hello-io-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredHelloIoTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_HELLO_IO_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredHelloIoTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredHelloIoTmpDir", () => {
  it("prefers /tmp/hello-io when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_HELLO_IO_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/hello-io when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_HELLO_IO_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/hello-io when /tmp/hello-io is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/hello-io when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/hello-io is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/hello-io is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/hello-io is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback HelloIo temp dir/);
  });
