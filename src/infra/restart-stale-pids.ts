 ...
 * returning control to the caller (typically `triggerHelloIoRestart` →
...
/**
 * Parse hello-io gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("hello-io")) {
 ...
 * Returns only PIDs that belong to hello-io gateway processes (not the current process).
 */
