...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.HELLO_IO_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for HelloIo. You can keep running the web gateway without it: \`${formatCliCommand("hello-io gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
