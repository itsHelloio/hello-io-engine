...
const DEFAULT_PACKAGE_NAME = "hello-io";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["hello-io", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "hello-io.mjs");
    ...
    const doctorStep = await runStep(
      step("hello-io doctor", doctorArgv, gitRoot, { HELLO_IO_UPDATE_IN_PROGRESS: "1" }),
    );
