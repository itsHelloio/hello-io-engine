...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__hello-io_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "hello-io-root");
...
describe("resolveHelloIoPackageRoot", () => {
  let resolveHelloIoPackageRoot: typeof import("./hello-io-root.js").resolveHelloIoPackageRoot;
  let resolveHelloIoPackageRootSync: typeof import("./hello-io-root.js").resolveHelloIoPackageRootSync;

  beforeAll(async () => {
    ({ resolveHelloIoPackageRoot, resolveHelloIoPackageRootSync } =
      await import("./hello-io-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "hello-io");
    const pkgRoot = path.join(project, "node_modules", "hello-io");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "hello-io" }));

    expect(resolveHelloIoPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "hello-io");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "hello-io.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "hello-io" }));

    expect(resolveHelloIoPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveHelloIoPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveHelloIoPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveHelloIoPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-hello-io package roots", async () => {
    const pkgRoot = fx("not-hello-io");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-hello-io" }));

    expect(resolveHelloIoPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveHelloIoPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveHelloIoPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
