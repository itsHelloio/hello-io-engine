...
import { ensureHelloIoExecMarkerOnProcess } from "./infra/hello-io-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "hello-io.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "hello-io.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "hello-io";
  ensureHelloIoExecMarkerOnProcess();
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.HELLO_IO_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.HELLO_IO_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.HELLO_IO_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.HELLO_IO_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[hello-io] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `HelloIo ${VERSION} (${commit})` : `HelloIo ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[hello-io] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[hello-io] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[hello-io] ${parsed.error}`);
      process.exit(2);
    }
