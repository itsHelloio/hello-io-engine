...
type DiscordConfig = NonNullable<
  import("../../config/config.js").HelloIoConfig["channels"]
>["discord"];
...
} as import("../../config/config.js").HelloIoConfig;
...
function createPreflightArgs(params: {
  cfg: import("../../config/config.js").HelloIoConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createMessage({
      ...
      channelId,
      content: "hi <@hello-io-bot>",
      mentionedUsers: [{ id: "hello-io-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey hello-io");
