...
function resolveVoiceTtsConfig(params: { cfg: HelloIoConfig; override?: TtsConfig }): {
  cfg: HelloIoConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredHelloIoTmpDir(), "discord-voice-"));
