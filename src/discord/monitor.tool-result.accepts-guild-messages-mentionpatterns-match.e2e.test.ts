  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/hello-io",
      },
    },
    session: { store: "/tmp/hello-io-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bhello-io\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "hello-io: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "HelloIo" },
          },
        },
        eventPatch: {
