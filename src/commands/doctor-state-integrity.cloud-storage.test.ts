...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "HelloIo", ".hello-io");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/hello-io-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".hello-io");
  ...
  it("anchors cloud detection to OS homedir when HELLO_IO_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".hello-io");
    const originalHelloIoHome = process.env.HELLO_IO_HOME;
    process.env.HELLO_IO_HOME = "/tmp/hello-io-home-override";
    ...
    try {
      ...
      if (originalHelloIoHome === undefined) {
        delete process.env.HELLO_IO_HOME;
      } else {
        process.env.HELLO_IO_HOME = originalHelloIoHome;
      }
    }
  });
