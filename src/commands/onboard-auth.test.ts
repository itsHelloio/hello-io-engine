...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under HELLO_IO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("hello-io-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-oauth-sync-"));
    ...
    process.env.HELLO_IO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-oauth-nosync-"));
    process.env.HELLO_IO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside HELLO_IO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-oauth-external-"));
    process.env.HELLO_IO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* HELLO_IO_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to HELLO_IO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("hello-io-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
