...
describe("backup commands", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("hello-io-backup-test-");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-workspace-link-"));
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, "hello-io.json"),
        JSON.stringify({
          agents: {
            defaults: {
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    ...
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backups-"));
    try {
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    ...
    const archiveDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "hello-io-backup-verify-on-create-"),
    );
    try {
      await fs.writeFile(path.join(stateDir, "hello-io.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".hello-io");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".hello-io");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-cwd-link-"));
