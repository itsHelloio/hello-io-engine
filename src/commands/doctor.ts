...
import { resolveHelloIoPackageRoot } from "../infra/hello-io-root.js";
...
function resolveMode(cfg: HelloIoConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("HelloIo doctor");

  const root = await resolveHelloIoPackageRoot({
  ...
  let cfg: HelloIoConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("hello-io configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("hello-io config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("hello-io setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("hello-io config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("hello-io config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("hello-io doctor --fix")}" to apply changes.`);
  }
