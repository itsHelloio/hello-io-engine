...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "HELLO_IO_GATEWAY_TOKEN",
          placeholder: "HELLO_IO_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like HELLO_IO_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. HelloIo will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: HelloIo trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.hello-io.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
