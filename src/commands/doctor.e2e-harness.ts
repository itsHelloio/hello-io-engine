...
export const resolveHelloIoPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadHelloIoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveHelloIoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.HELLO_IO_STATE_DIR;
  originalUpdateInProgress = process.env.HELLO_IO_UPDATE_IN_PROGRESS;
  process.env.HELLO_IO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.HELLO_IO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.HELLO_IO_UPDATE_IN_PROGRESS;
  } else {
    process.env.HELLO_IO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
