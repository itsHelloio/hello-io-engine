...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("hello-io-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".hello-io");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "hello-io.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-no-manifest-"));
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-hello-io-backup";
      ...
      const manifest = {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.hello-io",
            archivePath: `${rootName}/payload/posix/tmp/.hello-io`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-traversal-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
  ...
  it("fails when archive paths contain backslashes", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-backslash-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".hello-io");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-duplicate-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.hello-io",
            archivePath: `${rootName}/payload/posix/tmp/.hello-io/payload.txt`,
          },
        ],
      };
      await fs.writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
      ...
      await tar.c(
        {
          ...
          onWriteEntry: (entry) => {
            ...
            if (entry.path === payloadPath) {
              entry.path = `${rootName}/payload/posix/tmp/.hello-io/payload.txt`;
            }
          },
        },
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hello-io-backup-duplicate-payload-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    ...
    try {
      const rootName = "2026-03-09T00-00-00.000Z-hello-io-backup";
      const payloadArchivePath = `${rootName}/payload/posix/tmp/.hello-io/payload.txt`;
      const manifest = {
        schemaVersion: 1,
        createdAt: "2026-03-09T00:00:00.000Z",
        archiveRoot: rootName,
