...
import { HelloIoSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: HelloIoConfig): {
  config: HelloIoConfig;
  removed: string[];
} {
  const parsed = HelloIoSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: HelloIoConfig): void {
