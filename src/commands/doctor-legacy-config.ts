...
export function normalizeCompatibilityConfigValues(cfg: HelloIoConfig): {
  config: HelloIoConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: HelloIoConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as HelloIoConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as HelloIoConfig["browser"],
    };
    changes.push(
