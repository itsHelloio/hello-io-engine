...
export async function loadValidConfigOrThrow(): Promise<HelloIoConfig> {
...
export async function updateConfig(
  mutator: (cfg: HelloIoConfig) => HelloIoConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: HelloIoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: HelloIoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("hello-io agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
