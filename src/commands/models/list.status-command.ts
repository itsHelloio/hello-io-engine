...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveHelloIoAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("hello-io models auth setup-token")}\` or \`${formatCliCommand("hello-io configure")}\`.`
          : `Run \`${formatCliCommand("hello-io configure")}\` or set an API key env var.`;
