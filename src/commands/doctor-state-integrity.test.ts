...
type EnvSnapshot = {
  HOME?: string;
  HELLO_IO_HOME?: string;
  HELLO_IO_STATE_DIR?: string;
  HELLO_IO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    HELLO_IO_HOME: process.env.HELLO_IO_HOME,
    HELLO_IO_STATE_DIR: process.env.HELLO_IO_STATE_DIR,
    HELLO_IO_OAUTH_DIR: process.env.HELLO_IO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: HelloIoConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: HelloIoConfig) {
...
async function runStateIntegrityText(cfg: HelloIoConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.HELLO_IO_HOME = tempHome;
    process.env.HELLO_IO_STATE_DIR = path.join(tempHome, ".hello-io");
    delete process.env.HELLO_IO_OAUTH_DIR;
    fs.mkdirSync(process.env.HELLO_IO_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when HELLO_IO_OAUTH_DIR is explicitly configured", async () => {
    process.env.HELLO_IO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: HelloIoConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints hello-io-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: HelloIoConfig = {};
    ...
    expect(text).toMatch(/hello-io sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/hello-io sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /hello-io sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
