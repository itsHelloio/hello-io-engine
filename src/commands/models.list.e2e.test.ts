...
const ensureHelloIoModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveHelloIoAgentDir = vi.fn().mockReturnValue("/tmp/hello-io-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureHelloIoModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveHelloIoAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureHelloIoModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureHelloIoModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureHelloIoModelsJson).toHaveBeenCalled();
    expect(ensureHelloIoModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
