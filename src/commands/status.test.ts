...
beforeAll(() => {
  envSnapshot = captureEnv(["HELLO_IO_PROFILE"]);
  process.env.HELLO_IO_PROFILE = "isolated";
});
...
vi.mock("../infra/hello-io-root.js", () => ({
  resolveHelloIoPackageRoot: vi.fn().mockResolvedValue("/tmp/hello-io"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/hello-io/pnpm-lock.yaml",
      markerPath: "/tmp/hello-io/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.hello-io.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.hello-io.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("hello-io status --all") ||
          line.includes("hello-io --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("HELLO_IO_GATEWAY_TOKEN", "abcd1234", async () => {
