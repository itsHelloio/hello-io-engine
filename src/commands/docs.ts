...
const SEARCH_TOOL = "https://docs.hello-io.ai/mcp.SearchHelloIo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.hello-io.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('hello-io docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.hello-io.ai/");
      runtime.log(`Search: ${formatCliCommand('hello-io docs "your query"')}`);
    }
    return;
  }
