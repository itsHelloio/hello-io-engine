...
describe("readServiceStatusSummary", () => {
  it("marks HelloIo-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["hello-io", "gateway", "run"] })),
    ...
    expect(summary.managedByHelloIo).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByHelloIo).toBe(false);
