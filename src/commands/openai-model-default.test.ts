...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: HelloIoConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: HelloIoConfig) => { changed: boolean; next: HelloIoConfig };
  defaultModel: string;
  overrideConfig: HelloIoConfig;
  alreadyDefaultConfig: HelloIoConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: HelloIoConfig) => config,
      applyDefaultConfig: (config: HelloIoConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: HelloIoConfig = { agents: { defaults: {} } };
