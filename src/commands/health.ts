...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.HELLO_IO_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: HelloIoConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.HELLO_IO_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
