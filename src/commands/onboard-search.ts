...
function rawKeyValue(config: HelloIoConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: HelloIoConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: HelloIoConfig, provider: SearchProvider): HelloIoConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: HelloIoConfig, result: HelloIoConfig): HelloIoConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with hello-io configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — HelloIo will store a reference instead of the API key.",
