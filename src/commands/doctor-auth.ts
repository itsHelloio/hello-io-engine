...
function pruneAuthProfiles(
  cfg: HelloIoConfig,
  profileIds: Set<string>,
): { next: HelloIoConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("hello-io models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("hello-io models auth setup-token")} or ${formatCliCommand(
      "hello-io configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "hello-io models auth login --provider openai-codex",
    )} or ${formatCliCommand("hello-io configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("hello-io configure")}\` or \`${formatCliCommand("hello-io onboard")}\`.`;
}
