...
import { readConfigFileSnapshot, writeConfigFile, type HelloIoConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("hello-io config set gateway.auth.mode token")} or ${formatCliCommand("hello-io config set gateway.auth.mode password")}.`,
  ].join(" ");
}
