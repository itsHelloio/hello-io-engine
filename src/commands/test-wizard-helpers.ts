...
export function requireHelloIoAgentDir(): string {
  const agentDir = process.env.HELLO_IO_AGENT_DIR;
  if (!agentDir) {
    throw new Error("HELLO_IO_AGENT_DIR not set");
  }
  return agentDir;
}
