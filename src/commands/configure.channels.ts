...
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in hello-io.json.",
          `Tip: \`${formatCliCommand("hello-io channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    next = {
      ...
      channels: Object.keys(nextChannels).length
        ? (nextChannels as HelloIoConfig["channels"])
        : undefined,
    };
