...
type PromptDefaultModelResult = { model?: string; config?: HelloIoConfig };
...
function resolveConfiguredModelRaw(cfg: HelloIoConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: HelloIoConfig): string[] {
...
export function applyPrimaryModel(cfg: HelloIoConfig, model: string): HelloIoConfig {
...
export function applyModelAllowlist(cfg: HelloIoConfig, models: string[]): HelloIoConfig {
  const defaults = cfg.agents?.defaults;
