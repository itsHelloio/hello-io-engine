...
import type { HelloIoConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: HelloIoConfig): string[] {
...
export async function noteSecurityWarnings(cfg: HelloIoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("hello-io security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.hello-io/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("hello-io approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.hello-io.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("hello-io configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("hello-io config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("hello-io doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("hello-io config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('hello-io config set session.dmScope "per-channel-peer"') +
