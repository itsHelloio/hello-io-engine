...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hello-io-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.HELLO_IO_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "hello-io.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadHelloIoPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          hello-io: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadHelloIoPlugins>) {
...
function useNoBundledPlugins() {
  process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadHelloIoPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.HELLO_IO_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.HELLO_IO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadHelloIoPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadHelloIoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadHelloIoPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadHelloIoPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "hello-io.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "hello-io.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ HELLO_IO_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("hello-io/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadHelloIoPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadHelloIoPlugins({
        cache: false,
    ...
    execFileSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", script], {
      ...
      env: {
        ...process.env,
        HELLO_IO_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
      encoding: "utf-8",
      stdio: "pipe",
    });
