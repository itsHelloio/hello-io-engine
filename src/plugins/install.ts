...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing hello-io.extensions; update the plugin package to include hello-io.extensions (for example ["./dist/index.js"]). See https://docs.hello-io.ai/help/troubleshooting#plugin-install-fails-with-missing-hello-io-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_HELLO_IO_EXTENSIONS: "missing_hello-io_extensions",
  EMPTY_HELLO_IO_EXTENSIONS: "empty_hello-io_extensions",
...
function ensureHelloIoExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_HELLO_IO_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json hello-io.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_HELLO_IO_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureHelloIoExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from hello-io.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-hello-io"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "hello-io security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "hello-io security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "hello-io-npm-pack-",
    spec,
    timeoutMs,
