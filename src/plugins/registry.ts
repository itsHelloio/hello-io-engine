...
import type {
  HelloIoPluginApi,
  HelloIoPluginChannelRegistration,
  HelloIoPluginCliRegistrar,
  HelloIoPluginCommandDefinition,
  HelloIoPluginHttpRouteAuth,
  HelloIoPluginHttpRouteMatch,
  HelloIoPluginHttpRouteHandler,
  HelloIoPluginHttpRouteParams,
  HelloIoPluginHookOptions,
  ProviderPlugin,
  HelloIoPluginService,
  HelloIoPluginToolContext,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: HelloIoPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: HelloIoPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: HelloIoPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: HelloIoPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: HelloIoPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: HelloIoPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: HelloIoPluginHookOptions | undefined,
    config: HelloIoPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: HelloIoPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: HelloIoPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as HelloIoPluginChannelRegistration).plugin === "object"
        ? (registration as HelloIoPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: HelloIoPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: HelloIoPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: HelloIoPluginCommandDefinition) => {
