...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `hello-io-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "hello-io.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "hello-io.plugin.json");
  const linkedManifest = path.join(rootDir, "hello-io.plugin.json");
