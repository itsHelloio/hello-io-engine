...
INSTALL_URL="${HELLO_IO_INSTALL_URL:-https://hello-io.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${HELLO_IO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${HELLO_IO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="hello-io"
PACKAGE_NAME="${HELLO_IO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (HELLO_IO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${HELLO_IO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${HELLO_IO_INSTALL_LATEST_OUT:-}"
fi
