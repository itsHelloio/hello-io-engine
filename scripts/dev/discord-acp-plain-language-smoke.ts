...
type DriverMode = "token" | "webhook" | "hello-io";
...
function resolveStateDir(): string {
  const override = process.env.HELLO_IO_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.HELLO_IO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".hello-io");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver hello-io] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) HelloIo spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|hello-io> Driver transport mode (default: token)\n" +
    ...
    "  --hello-io-bin <path>        HelloIo CLI binary for driver=hello-io (default: hello-io)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  HELLO_IO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  HELLO_IO_DISCORD_SMOKE_DRIVER\n" +
    "  HELLO_IO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  HELLO_IO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  HELLO_IO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  HELLO_IO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  HELLO_IO_DISCORD_SMOKE_AGENT\n" +
    "  HELLO_IO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  HELLO_IO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  HELLO_IO_DISCORD_SMOKE_POLL_MS\n" +
    "  HELLO_IO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  HELLO_IO_DISCORD_SMOKE_HELLO_IO_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.HELLO_IO_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.HELLO_IO_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.HELLO_IO_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.HELLO_IO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.HELLO_IO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.HELLO_IO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.HELLO_IO_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.HELLO_IO_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.HELLO_IO_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.HELLO_IO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.HELLO_IO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.HELLO_IO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const hello-ioBin =
    resolveArg("--hello-io-bin") || process.env.HELLO_IO_DISCORD_SMOKE_HELLO_IO_BIN || "hello-io";
...
async function hello-ioCliJson<T>(params: { hello-ioBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.hello-ioBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`hello-io ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await hello-ioCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    hello-ioBin: params.hello-ioBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "hello-io") {
    return await readMessagesWithOpenclaw({
      hello-ioBin: params.args.hello-ioBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `hello-io-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await hello-ioCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("hello-io message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from HelloIo.`,
        diagnostics: {
          bindingCandidates: [
            {
