 ...
 * `hello-io update`. Use it to verify the completion installation flow
...
\`hello-io update\`, \`hello-io doctor\`, and \`hello-io onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# HelloIo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`hello-io completion --install\` later to enable.`));
    return;
  }
