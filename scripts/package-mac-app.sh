...
# Build and bundle HelloIo into a minimal .app we can open.
# Outputs to dist/HelloIo.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/HelloIo.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="HelloIo"
BUNDLE_ID="${BUNDLE_ID:-ai.hello-io.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/hello-io/hello-io/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/HelloIo/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :HelloIoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :HelloIoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/HelloIo"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/HelloIo"
fi
chmod +x "$APP_ROOT/Contents/MacOS/HelloIo"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/HelloIo" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/HelloIo/Resources/HelloIo.icns" "$APP_ROOT/Contents/Resources/HelloIo.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/HelloIo/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying HelloIoKit resources"
HELLO_IOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/HelloIoKit_HelloIoKit.bundle"
if [ -d "$HELLO_IOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/HelloIoKit_HelloIoKit.bundle"
  cp -R "$HELLO_IOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/HelloIoKit_HelloIoKit.bundle"
else
  echo "WARN: HelloIoKit resource bundle not found at $HELLO_IOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running HelloIo"
killall -q HelloIo 2>/dev/null || true
