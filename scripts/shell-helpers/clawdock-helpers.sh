#!/usr/bin/env bash
# ClawDock - Docker helpers for HelloIo
# Inspired by Simon Willison's "Running HelloIo in Docker"
# https://til.simonwillison.net/llms/hello-io-docker
#
# Installation:
...
# Common paths to check for HelloIo
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/hello-io"
  "${HOME}/workspace/hello-io"
  "${HOME}/projects/hello-io"
  "${HOME}/dev/hello-io"
  "${HOME}/code/hello-io"
  "${HOME}/src/hello-io"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found HelloIo at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/hello-io"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ HelloIo not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/hello-io/hello-io.git ~/hello-io"
    echo "  cd ~/hello-io && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^HELLO_IO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d hello-io-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart hello-io-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f hello-io-gateway
}
...
clawdock-workspace() {
  cd ~/.hello-io/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec hello-io-gateway \
    bash -c 'echo "alias hello-io=\"./hello-io.mjs\"" > /tmp/.bashrc_hello-io && bash --rcfile /tmp/.bashrc_hello-io'
}
...
clawdock-cli() {
  _clawdock_compose run --rm hello-io-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build hello-io-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "HELLO_IO_GATEWAY_TOKEN=$token" hello-io-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" hello-io-gateway \
    bash -c './hello-io.mjs config set gateway.remote.token "$TOKEN" && ./hello-io.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec hello-io-gateway \
    bash -c "./hello-io.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart hello-io-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm hello-io-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec hello-io-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'hello-io config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for HelloIo${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (hello-io alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to hello-io project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.hello-io)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'hello-io channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'hello-io status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.hello-io.ai${_CLR_RESET}"
  echo ""
}
