...
IMAGE_NAME="hello-io-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    HELLO_IO_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    HELLO_IO_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export HELLO_IO_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/hello-io-bin:$PATH"
  mkdir -p /tmp/hello-io-bin
  cat > /tmp/hello-io-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/hello-io-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$HELLO_IO_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_hello-io_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/hello-io-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/hello-io-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$HELLO_IO_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/hello-io-e2e-$1.XXXXXX"
  }

  set_isolated_hello-io_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export HELLO_IO_HOME="$home_dir"
    export HELLO_IO_STATE_DIR="$home_dir/.hello-io"
    export HELLO_IO_CONFIG_PATH="$HELLO_IO_STATE_DIR/hello-io.json"
    mkdir -p "$HELLO_IO_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HELLO_IO_STATE_DIR/workspace"
    config_path="$HELLO_IO_CONFIG_PATH"
    sessions_dir="$HELLO_IO_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_hello-io_env "$home_dir"
	    ...
	    node "$HELLO_IO_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$HELLO_IO_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$HELLO_IO_ENTRY\" configure --section skills" send_skills_flow
