...
IMAGE_NAME="hello-io-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="hello-io-net-e2e-$$"
GW_NAME="hello-io-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "HELLO_IO_GATEWAY_TOKEN=$TOKEN" \
  -e "HELLO_IO_SKIP_CHANNELS=1" \
  -e "HELLO_IO_SKIP_GMAIL_WATCHER=1" \
  -e "HELLO_IO_SKIP_CRON=1" \
  -e "HELLO_IO_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
