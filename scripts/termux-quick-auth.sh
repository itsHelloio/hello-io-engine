...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/hello-io/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "HelloIo Re-Auth" -c "After getting key, run: ssh $SERVER '~/hello-io/scripts/mobile-reauth.sh'" --id hello-io-auth
        ;;
    *)
