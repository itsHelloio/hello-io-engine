...
BASE_IMAGE="${BASE_IMAGE:-hello-io-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-hello-io-sandbox-common:bookworm-slim}"
...
HELLO_IO_DOCKER_BUILD_USE_BUILDX="${HELLO_IO_DOCKER_BUILD_USE_BUILDX:-0}"
HELLO_IO_DOCKER_BUILD_CACHE_FROM="${HELLO_IO_DOCKER_BUILD_CACHE_FROM:-}"
HELLO_IO_DOCKER_BUILD_CACHE_TO="${HELLO_IO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${HELLO_IO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${HELLO_IO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${HELLO_IO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${HELLO_IO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${HELLO_IO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=hello-io.sandbox=1)
NOTE
