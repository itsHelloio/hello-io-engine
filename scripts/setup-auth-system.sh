#!/bin/bash
# Setup HelloIo Auth Management System
...
echo "=== HelloIo Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'hello-io-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  HelloIo message: Send warning via HelloIo itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/hello-io-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/hello-io-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/hello-io-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now hello-io-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status hello-io-auth-monitor.timer"
echo ""
echo "Quick commands:"
