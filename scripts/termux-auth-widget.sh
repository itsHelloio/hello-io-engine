...
# HelloIo Auth Widget for Termux
...
# Check auth status
termux-toast "Checking HelloIo auth..."

STATUS=$(ssh "$SERVER" '$HOME/hello-io/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/hello-io/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|HELLO_IO_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/hello-io/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|HELLO_IO_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: HelloIo won't work until re-auth"
                ;;
        esac
        ;;
