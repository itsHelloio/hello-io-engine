...
#
# Suggested cron: */30 * * * * /home/admin/hello-io/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send HelloIo notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., hello-io-alerts)
...
STATE_FILE="$HOME/.hello-io/auth-monitor-state"
...
send_notification() {
    ...
    # Send via HelloIo if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use hello-io
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via HelloIo to $NOTIFY_PHONE..."
            hello-io send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! HelloIo is down. Run: ssh l36 '~/hello-io/scripts/mobile-reauth.sh'" "urgent"
    exit 1
