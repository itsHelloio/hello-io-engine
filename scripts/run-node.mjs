...
const shouldBuild = (deps) => {
  if (deps.env.HELLO_IO_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.HELLO_IO_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[hello-io] ${message}\n`);
};

const runHelloIo = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["hello-io.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
