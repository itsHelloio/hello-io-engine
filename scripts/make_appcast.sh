...
ZIP=${1:?"Usage: $0 HelloIo-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/hello-io/hello-io/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^HelloIo-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/hello-io/hello-io/releases/download/v${VERSION}/"}
