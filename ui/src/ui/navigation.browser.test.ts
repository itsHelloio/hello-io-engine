...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/hello-io/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/hello-io");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/hello-io/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__HELLO_IO_CONTROL_UI_BASE_PATH__ = "/hello-io";
    const app = mountApp("/hello-io/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/hello-io");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/hello-io/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("hello-io.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/hello-io" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("hello-io.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/hello-io",
    });
    expect(JSON.parse(localStorage.getItem("hello-io.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/hello-io";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/hello-io");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/hello-io#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/hello-io");
    expect(app.settings.token).toBe("");
