...
export async function handleWhatsAppStart(host: HelloIoApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: HelloIoApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: HelloIoApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: HelloIoApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: HelloIoApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: HelloIoApp): string {
...
function resolveGatewayHttpAuthHeader(host: HelloIoApp): string | null {
...
function buildGatewayHttpHeaders(host: HelloIoApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: HelloIoApp) {
...
export function handleNostrProfileToggleAdvanced(host: HelloIoApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: HelloIoApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: HelloIoApp) {
  const state = host.nostrProfileFormState;
