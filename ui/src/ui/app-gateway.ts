...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    clientName: "hello-io-control-ui",
    clientVersion,
    mode: "webchat",
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as HelloIoApp);
      void loadAgents(host as unknown as HelloIoApp);
      void loadToolsCatalog(host as unknown as HelloIoApp);
      void loadNodes(host as unknown as HelloIoApp, { quiet: true });
      void loadDevices(host as unknown as HelloIoApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as HelloIoApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as HelloIoApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as HelloIoApp, payload);
