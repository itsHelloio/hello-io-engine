...
read_when:
  - Designing workspace memory (~/.hello-io/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep HelloIo integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.hello-io/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.hello-io/workspace` as canonical human-readable memory.
...
```
~/.hello-io/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in HelloIo**, but keep a separable core library.

### Why integrate into HelloIo?

- HelloIo already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `hello-io memory recall "…" --k 25 --since 30d`
  - `hello-io memory reflect --since 7d`
...
Pragmatic take for `~/.hello-io/workspace`:
