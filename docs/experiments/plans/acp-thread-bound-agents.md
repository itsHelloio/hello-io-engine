...
This plan defines how HelloIo should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- HelloIo core owns ACP control plane concerns
...
HelloIo should not reimplement ACP transport internals in core.
HelloIo should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in HelloIo, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under HelloIo state dir
...
- ACP event stream is normalized to HelloIo reply chunks
...
For the first production backend (`extensions/acpx`), HelloIo and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses HelloIo `sessionKey`)
...
- HelloIo consumes ndjson events from `acpx --format json --json-strict`
