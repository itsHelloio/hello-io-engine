...
service via the external `hello-io` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.hello-io.gateway`
  (or `ai.hello-io.<profile>` when using `--profile`/`HELLO_IO_PROFILE`; legacy `com.hello-io.*` is supported).
