...
- Same path: run the app from a fixed location (for HelloIo, `dist/HelloIo.app`).
...
```bash
sudo tccutil reset Accessibility ai.hello-io.mac
sudo tccutil reset ScreenCapture ai.hello-io.mac
...
Workaround: move files into the HelloIo workspace (`~/.hello-io/workspace`) if you want to avoid per-folder grants.
