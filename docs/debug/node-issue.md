...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in HelloIo
...
Running HelloIo via Node with `tsx` fails at startup with:

```
[hello-io] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hello-io status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch hello-io.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node hello-io.mjs status` works on Node 25.
