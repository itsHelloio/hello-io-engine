...
description: "Complete field-by-field reference for ~/.hello-io/hello-io.json"
summary: "Complete reference for every HelloIo config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.hello-io/hello-io.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — HelloIo uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.hello-io/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `hello-io doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `hello-io doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["hello-io-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-hello-io",
          requireMention: false,
...
- HelloIo additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `hello-io plugins install @hello-io/mattermost`.
...
- `commands.callbackUrl` must resolve to the HelloIo gateway endpoint and be reachable from the Mattermost server.
...
HelloIo spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `hello-io channels add` (or channel onboarding) while still on a single-account top-level channel config, HelloIo moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `hello-io doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@hello-io", "hello-io"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@hello-io"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `hello-io.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, HelloIo auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/hello-io" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, HelloIo assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "hello-io-sandbox-browser:bookworm-slim",
          network: "hello-io-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.hello-io/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `hello-io.json`.
noVNC observer access uses VNC auth by default and HelloIo emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `hello-io-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `HELLO_IO_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `HELLO_IO_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `HELLO_IO_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.hello-io/workspace",
        agentDir: "~/.hello-io/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@hello-io"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.hello-io/workspace-home" },
      { id: "work", workspace: "~/.hello-io/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, HelloIo resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.hello-io/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.hello-io/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.hello-io/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.hello-io/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, HelloIo starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.hello-io/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, HelloIo treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:hello-io`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.hello-io/attachments/<uuid>/` with a `.manifest.json`.
...
HelloIo uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.hello-io/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `HELLO_IO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), HelloIo forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `hello-io onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `hello-io onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `hello-io onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `hello-io onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `hello-io onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `hello-io onboard --auth-choice minimax-api`.
...
- Loaded from `~/.hello-io/extensions`, `<workspace>/.hello-io/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `hello-io plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      hello-io: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or HELLO_IO_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `HELLO_IO_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `HELLO_IO_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.hello-io-dev` + port `19001`), `--profile <name>` (uses `~/.hello-io-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.hello-io/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-hello-io-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `HELLO_IO_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.hello-io/workspace/canvas",
    liveReload: true,
    // enabled: false, // or HELLO_IO_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__hello-io__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__hello-io__/a2ui/`
...
- Also serves A2UI at `/__hello-io__/a2ui/`.
...
- Hostname defaults to `hello-io`. Override with `HELLO_IO_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.hello-io/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `hello-io dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.hello-io/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${HELLO_IO_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `hello-io.json` credential paths.
...
- Legacy OAuth imports from `~/.hello-io/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/hello-io/hello-io.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/hello-io/hello-io-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one HelloIo.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `HELLO_IO_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `hello-io doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `hello-io.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
