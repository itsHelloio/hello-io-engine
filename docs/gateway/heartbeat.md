...
- During heartbeat runs, HelloIo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `hello-io sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, HelloIo skips the heartbeat run entirely (no model call).
...
```bash
hello-io system event --text "Check for urgent follow-ups" --mode now
```
