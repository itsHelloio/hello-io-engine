---
summary: "How HelloIo sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
HelloIo can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`hello-io-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; HelloIo emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- `"none"` (default): tools see a sandbox workspace under `~/.hello-io/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
HelloIo mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- HelloIo blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `hello-io-sandbox:bookworm-slim`
...
`hello-io-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `HELLO_IO_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `HELLO_IO_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `HELLO_IO_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `HELLO_IO_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `HELLO_IO_DOCKER_SOCKET`. Full setup and env
...
- Use `hello-io sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
