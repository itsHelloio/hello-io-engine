...
read_when:
  - Running HelloIo behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of HelloIo
...
- You run HelloIo behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. HelloIo checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. HelloIo extracts the user identity from the configured header
...
- HelloIo can stay on loopback HTTP behind the proxy.
...
If HelloIo itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://hello-io.example.com
    to: http://hello-io-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
hello-io.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy hello-io:18789 {
...
location / {
    ...
    proxy_pass http://hello-io:18789;
...
`hello-io security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update HelloIo config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `hello-io security audit` and review findings
