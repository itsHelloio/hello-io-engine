---
summary: "VPS hosting hub for HelloIo (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/hello-io-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `HELLO_IO_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `HELLO_IO_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/hello-io-compile-cache`
