---
summary: "CLI reference for `hello-io configure` (interactive configuration prompts)"
read_when:
...
Tip: `hello-io config` without a subcommand opens the same wizard. Use
`hello-io config get|set|unset` for non-interactive edits.
...
```bash
hello-io configure
hello-io configure --section model --section channels
```
