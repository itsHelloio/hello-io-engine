---
summary: "CLI reference for `hello-io onboard` (interactive onboarding wizard)"
read_when:
...
```bash
hello-io onboard
hello-io onboard --flow quickstart
hello-io onboard --flow manual
hello-io onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`HELLO_IO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `hello-io dashboard` (Control UI, no channel setup).
