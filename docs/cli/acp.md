...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a HelloIo Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`hello-io acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the HelloIo gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
hello-io acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
hello-io acp --url wss://gateway-host:18789 --token-file ~/.hello-io/gateway.token

# Attach to an existing session key
hello-io acp --session agent:main:main

# Attach by label (must already exist)
hello-io acp --session-label "support inbox"

# Reset the session key before the first prompt
hello-io acp --session agent:main:main --reset-session
```
...
hello-io acp client --server-args --url wss://gateway-host:18789 --token-file ~/.hello-io/gateway.token

# Override the server command (default: hello-io)
hello-io acp client --server "node" --server-args hello-io.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a HelloIo Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `hello-io acp` over stdio.

Example config (persisted):

```bash
hello-io config set gateway.remote.url wss://gateway-host:18789
hello-io config set gateway.remote.token <token>
```
...
```bash
hello-io acp --session agent:main:main
hello-io acp --session agent:design:main
hello-io acp --session agent:qa:bug-123
```
...
HelloIo bot over ACP, use `acpx` with its built-in `hello-io` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx hello-io` at `hello-io acp`.
3. Target the HelloIo session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default HelloIo ACP session
acpx hello-io exec "Summarize the active HelloIo session state."

# Persistent named session for follow-up turns
acpx hello-io sessions ensure --name codex-bridge
acpx hello-io -s codex-bridge --cwd /path/to/repo \
  "Ask my HelloIo work agent for recent context relevant to this repo."
```

If you want `acpx hello-io` to target a specific Gateway and session key every
time, override the `hello-io` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "hello-io": {
      "command": "env HELLO_IO_HIDE_BANNER=1 HELLO_IO_SUPPRESS_NOTES=1 hello-io acp --url ws://127.0.0.1:18789 --token-file ~/.hello-io/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local HelloIo checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env HELLO_IO_HIDE_BANNER=1 HELLO_IO_SUPPRESS_NOTES=1 node hello-io.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an HelloIo agent without scraping a terminal.
...
In Zed, open the Agent panel and select “HelloIo ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`HELLO_IO_GATEWAY_TOKEN`, `HELLO_IO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`HELLO_IO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback when `gateway.auth.*` is unset
...
- ACP runtime backend child processes receive `HELLO_IO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `hello-io acp client` sets `HELLO_IO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `hello-io`).
