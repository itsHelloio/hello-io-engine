---
summary: "CLI reference for `hello-io security` (audit and fix common security footguns)"
read_when:
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that HelloIo is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `hello-io.browserConfigEpoch`) and recommends `hello-io sandbox recreate --browser --all`.
...
```bash
hello-io security audit --json | jq '.summary'
hello-io security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
hello-io security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
