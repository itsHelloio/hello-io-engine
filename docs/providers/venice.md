---
summary: "Use Venice AI privacy-focused models in HelloIo"
read_when:
  - You want privacy-focused inference in HelloIo
...
```bash
hello-io onboard --auth-choice venice-api-key
```
...
```bash
hello-io agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, HelloIo shows all available Venice models. Pick based on your needs:
...
```bash
hello-io models set venice/kimi-k2-5
hello-io models set venice/claude-opus-4-6
```

List all available models:

```bash
hello-io models list | grep venice
```

## Configure via `hello-io configure`

1. Run `hello-io configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
HelloIo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
hello-io agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
hello-io agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
hello-io agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
hello-io agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
hello-io agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `hello-io models list` to see currently available models. Some models may be temporarily offline.
