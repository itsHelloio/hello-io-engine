---
summary: "Use OpenAI via API keys or Codex subscription in HelloIo"
read_when:
  - You want to use OpenAI models in HelloIo
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like HelloIo.
...
```bash
hello-io onboard --auth-choice openai-api-key
# or non-interactive
hello-io onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. HelloIo forwards both through the `openai/*` Responses path.
...
hello-io onboard --auth-choice openai-codex

# Or run OAuth directly
hello-io models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. HelloIo
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

HelloIo uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), HelloIo also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. HelloIo enables it by default for
...
HelloIo, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), HelloIo now auto-enables OpenAI server-side
