---
summary: "Use MiniMax M2.5 in HelloIo"
read_when:
  - You want MiniMax models in HelloIo
...
```bash
hello-io plugins enable minimax-portal-auth  # skip if already loaded.
hello-io gateway restart  # restart if gateway is already running
hello-io onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/hello-io/hello-io/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `hello-io.json`:
...
- Use `hello-io models list` and `hello-io models set minimax/MiniMax-M2.5` to switch.
...
- Running `hello-io configure` and selecting **MiniMax M2.5**, or
