---
summary: "Use Synthetic's Anthropic-compatible API in HelloIo"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. HelloIo registers it as the
...
Note: HelloIo's Anthropic client appends `/v1` to the base URL, so use
