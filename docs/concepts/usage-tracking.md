...
- `/usage cost` in chats: local cost summary aggregated from HelloIo session logs.
- CLI: `hello-io status --usage` prints a full per-provider breakdown.
- CLI: `hello-io channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
