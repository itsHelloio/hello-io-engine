...
HelloIo features that can generate provider usage or paid API calls.
...
- `hello-io status --usage` and `hello-io channels list` show provider **usage windows**
...
HelloIo can pick up credentials from:
...
- `hello-io status --usage`
- `hello-io models status --json`
...
`hello-io models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
