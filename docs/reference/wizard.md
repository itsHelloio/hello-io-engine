...
This is the full reference for the `hello-io onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.hello-io/credentials/oauth.json`; auth profiles live in `~/.hello-io/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.hello-io/credentials/oauth.json` (or `$HELLO_IO_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.hello-io/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `hello-io pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `hello-io configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `hello-io health`.
    - Tip: `hello-io status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export HELLO_IO_GATEWAY_TOKEN="your-token"
hello-io onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env HELLO_IO_GATEWAY_TOKEN
```
...
- Stores it under `~/.hello-io/tools/signal-cli/<version>/`.
...
Typical fields in `~/.hello-io/hello-io.json`:
...
`hello-io agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.hello-io/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.hello-io/agents/<agentId>/sessions/`.
