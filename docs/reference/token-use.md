---
summary: "How HelloIo builds prompt context and reports token usage + costs"
read_when:
...
HelloIo tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

HelloIo assembles its own system prompt on every run. It includes:
...
For images, HelloIo downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from HelloIo session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `hello-io status --usage` and `hello-io channels list` show
...
`cacheWrite`. If pricing is missing, HelloIo shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. HelloIo can
...
Anthropic's 1M context window is currently beta-gated. HelloIo can inject the
...
HelloIo skips the `context-1m-*` beta header because Anthropic currently
