...
- With Anthropic API-key auth profiles, HelloIo seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, HelloIo injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
HelloIo exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.hello-io/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$HELLO_IO_STATE_DIR/logs/cache-trace.jsonl`
...
- `HELLO_IO_CACHE_TRACE=1` enables cache tracing.
- `HELLO_IO_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `HELLO_IO_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `HELLO_IO_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `HELLO_IO_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
