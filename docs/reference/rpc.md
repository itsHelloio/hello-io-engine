...
HelloIo integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- HelloIo owns lifecycle when `channels.signal.autoStart=true`.
...
- HelloIo spawns `imsg rpc` as a child process (legacy iMessage integration).
