...
你可以在 `~/.hello-io/hello-io.json` 中覆盖此设置：
...
所有日志配置都在 `~/.hello-io/hello-io.json` 的 `logging` 下。
...
- HelloIo 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `hello-io plugins enable diagnostics-otel` 启用插件。
...
- `hello-io.tokens`（计数器，属性：`hello-io.token`、`hello-io.channel`、`hello-io.provider`、`hello-io.model`）
- `hello-io.cost.usd`（计数器，属性：`hello-io.channel`、`hello-io.provider`、`hello-io.model`）
- `hello-io.run.duration_ms`（直方图，属性：`hello-io.channel`、`hello-io.provider`、`hello-io.model`）
- `hello-io.context.tokens`（直方图，属性：`hello-io.context`、`hello-io.channel`、`hello-io.provider`、`hello-io.model`）

消息流：

- `hello-io.webhook.received`（计数器，属性：`hello-io.channel`、`hello-io.webhook`）
- `hello-io.webhook.error`（计数器，属性：`hello-io.channel`、`hello-io.webhook`）
- `hello-io.webhook.duration_ms`（直方图，属性：`hello-io.channel`、`hello-io.webhook`）
- `hello-io.message.queued`（计数器，属性：`hello-io.channel`、`hello-io.source`）
- `hello-io.message.processed`（计数器，属性：`hello-io.channel`、`hello-io.outcome`）
- `hello-io.message.duration_ms`（直方图，属性：`hello-io.channel`、`hello-io.outcome`）

队列 + 会话：

- `hello-io.queue.lane.enqueue`（计数器，属性：`hello-io.lane`）
- `hello-io.queue.lane.dequeue`（计数器，属性：`hello-io.lane`）
- `hello-io.queue.depth`（直方图，属性：`hello-io.lane` 或 `hello-io.channel=heartbeat`）
- `hello-io.queue.wait_ms`（直方图，属性：`hello-io.lane`）
- `hello-io.session.state`（计数器，属性：`hello-io.state`、`hello-io.reason`）
- `hello-io.session.stuck`（计数器，属性：`hello-io.state`）
- `hello-io.session.stuck_age_ms`（直方图，属性：`hello-io.state`）
- `hello-io.run.attempt`（计数器，属性：`hello-io.attempt`）
...
- `hello-io.webhook.error`
  - `hello-io.channel`、`hello-io.webhook`、`hello-io.chatId`、`hello-io.error`
- `hello-io.message.processed`
  - `hello-io.channel`、`hello-io.outcome`、`hello-io.chatId`、`hello-io.messageId`、`hello-io.sessionKey`、`hello-io.sessionId`、`hello-io.reason`
- `hello-io.session.stuck`
  - `hello-io.state`、`hello-io.ageMs`、`hello-io.queueDepth`、`hello-io.sessionKey`、`hello-io.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `hello-io doctor`。
