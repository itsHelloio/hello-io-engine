...
HelloIo can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere HelloIo can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, HelloIo defaults
...
TTS config lives under `messages.tts` in `hello-io.json`.
...
- If `provider` is **unset**, HelloIo prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.hello-io/settings/tts.json`, override with `HELLO_IO_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, HelloIo retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, HelloIo:
...
Discord note: `/tts` is a built-in Discord command, so HelloIo registers
