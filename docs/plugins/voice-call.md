...
read_when:
  - You want to place an outbound voice call from HelloIo
...
Voice calls for HelloIo via a plugin. Supports outbound notifications and
...
- Use `hello-io voicecall ...` or the `voice_call` tool
...
```bash
hello-io plugins install ./extensions/voice-call
