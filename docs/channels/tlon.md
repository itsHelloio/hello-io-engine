...
Tlon is a decentralized messenger built on Urbit. HelloIo connects to your Urbit ship and can
...
```bash
hello-io plugins install @hello-io/tlon
```

Local checkout (when running from a git repo):

```bash
hello-io plugins install ./extensions/tlon
```
...
By default, HelloIo blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `hello-io message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, HelloIo replies in-thread.
