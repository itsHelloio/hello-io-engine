...
```bash
hello-io plugins install @hello-io/msteams
```

Local checkout (when running from a git repo):

```bash
hello-io plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
HelloIo will offer the local install path automatically.
...
3. Configure HelloIo with those credentials.
...
- Talk to HelloIo via Teams DMs, group chats, or channels.
...
- On startup, HelloIo resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.hello-io/hello-io.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring HelloIo, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `hello-io-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `hello-io plugins install @hello-io/msteams`
   - From a local checkout: `hello-io plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "HelloIo in Teams", "full": "HelloIo in Teams" },
...
HelloIo handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, HelloIo only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/HelloIoShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

HelloIo sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `hello-io message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.hello-io/msteams-polls.json`.
...
```bash
hello-io message send --channel msteams \
...
hello-io message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
hello-io message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
hello-io message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
hello-io message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
