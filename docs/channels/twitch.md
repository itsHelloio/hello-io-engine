...
read_when:
  - Setting up Twitch chat integration for HelloIo
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. HelloIo connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
hello-io plugins install @hello-io/twitch
```

Local checkout (when running from a git repo):

```bash
hello-io plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `HELLO_IO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "hello-io", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use HELLO_IO_TWITCH_ACCESS_TOKEN env var)
...
```bash
HELLO_IO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
