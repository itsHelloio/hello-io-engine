...
read_when:
  - Setting up Zalo Personal for HelloIo
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside HelloIo.
...
- Install via CLI: `hello-io plugins install @hello-io/zalouser`
- Or from a source checkout: `hello-io plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `hello-io channels login --channel zalouser`
...
```bash
hello-io directory self --channel zalouser
hello-io directory peers list --channel zalouser --query "name"
hello-io directory groups list --channel zalouser --query "work"
```
...
- `hello-io pairing list zalouser`
- `hello-io pairing approve zalouser <code>`
...
- On startup, HelloIo resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, HelloIo stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in HelloIo state. Example:
...
- HelloIo sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, HelloIo sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `hello-io channels logout --channel zalouser && hello-io channels login --channel zalouser`
...
- The channel now runs fully in HelloIo without external CLI binaries.
