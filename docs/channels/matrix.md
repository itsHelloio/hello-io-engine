...
Matrix is an open, decentralized messaging protocol. HelloIo connects as a Matrix **user**
...
```bash
hello-io plugins install @hello-io/matrix
```

Local checkout (when running from a git repo):

```bash
hello-io plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `hello-io plugins install @hello-io/matrix`
   - From a local checkout: `hello-io plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: HelloIo calls the same
     login endpoint, stores the access token in `~/.hello-io/credentials/matrix/credentials.json`,
...
- On first connection, HelloIo requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  HelloIo logs a warning.
...
`~/.hello-io/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `hello-io pairing list matrix`
  - `hello-io pairing approve matrix <CODE>`
...
- On startup, HelloIo resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
