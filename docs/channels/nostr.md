...
read_when:
  - You want HelloIo to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables HelloIo to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`hello-io onboard`) and `hello-io channels add` list optional channel plugins.
...
```bash
hello-io plugins install @hello-io/nostr
```

Use a local checkout (dev workflows):

```bash
hello-io plugins install --link <path-to-hello-io>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "hello-io@example.com",
        "lud16": "hello-io@example.com"
      }
    }
  }
}
