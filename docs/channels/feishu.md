...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects HelloIo to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current HelloIo releases, so no separate plugin install
is required.
...
```bash
hello-io plugins install @hello-io/feishu
```
...
If you just installed HelloIo, run the wizard:
...
2. Configuring app credentials in HelloIo
...
1. You already ran `hello-io channels add` for Feishu
2. The gateway is running (`hello-io gateway status`)
...
```bash
hello-io pairing approve feishu <CODE>
```
...
2. Run `hello-io logs --follow` and look for `chat_id`
...
2. Run `hello-io logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `hello-io gateway status`  | Show gateway status           |
| `hello-io gateway install` | Install/start gateway service |
| `hello-io gateway stop`    | Stop gateway service          |
| `hello-io gateway restart` | Restart gateway service       |
| `hello-io logs --follow`   | Tail gateway logs             |
...
4. Check logs: `hello-io logs --follow`
...
5. Ensure the gateway is running: `hello-io gateway status`
6. Check logs: `hello-io logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.hello-io/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.hello-io/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
