...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure HelloIo HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `hello-io pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while HelloIo is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
HelloIo supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, HelloIo falls back to normal delivery for remaining payloads.
