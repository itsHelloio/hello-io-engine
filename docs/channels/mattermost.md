---
summary: "Mattermost bot setup and HelloIo config"
read_when:
...
```bash
hello-io plugins install @hello-io/mattermost
```

Local checkout (when running from a git repo):

```bash
hello-io plugins install ./extensions/mattermost
```
...
4. Configure HelloIo and start the gateway.
...
Native slash commands are opt-in. When enabled, HelloIo registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, HelloIo derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as HelloIo.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to HelloIo.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from HelloIo, not `404`.
...
- Approve via:
  - `hello-io pairing list mattermost`
  - `hello-io pairing approve mattermost <CODE>`
...
Use these target formats with `hello-io message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, HelloIo derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and HelloIo run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="hello-io-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"hello-io-mattermost-interactions",
...
`hello-io message send` and cron/webhook deliveries.
