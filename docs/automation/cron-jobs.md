...
- Jobs persist under `~/.hello-io/cron/` so restarts don’t lose schedules.
...
- For upgrades, `hello-io doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
hello-io cron list
hello-io cron run <job-id>
hello-io cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.hello-io/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `hello-io cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, HelloIo applies a
...
If `delivery` is omitted for isolated jobs, HelloIo defaults to `announce`.
...
- CLI equivalents: `hello-io cron add --light-context ...` and `hello-io cron edit --light-context`.
...
- Job store: `~/.hello-io/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.hello-io/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, HelloIo classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, HelloIo archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `hello-io cron runs` and adjust retention before logs become large
...
hello-io cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
hello-io cron edit <jobId> --agent ops
hello-io cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
hello-io cron run <jobId>
hello-io cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `hello-io cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
hello-io cron edit <jobId> \
...
```bash
hello-io cron edit <jobId> --exact
```
...
```bash
hello-io system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`hello-io system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `HELLO_IO_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- HelloIo applies exponential retry backoff for recurring jobs after consecutive errors:
