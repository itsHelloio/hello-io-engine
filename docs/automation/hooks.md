...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in HelloIo.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in HelloIo. See [Webhook Hooks](/automation/webhook) or use `hello-io webhooks` for Gmail helper commands.
...
- Extend HelloIo's behavior without modifying core code

## Getting Started

### Bundled Hooks

HelloIo ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.hello-io/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.hello-io/logs/commands.log`
...
During onboarding (`hello-io onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.hello-io/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<hello-io>/dist/hooks/bundled/` (shipped with HelloIo)
...
Hook packs are standard npm packages that export one or more hooks via `hello-io.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.hello-io/hooks/<id>`.
Each `hello-io.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `hello-io hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.hello-io.ai/automation/hooks#my-hook
metadata:
  { "hello-io": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.hello-io` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before HelloIo persists them.
...
- **Managed hooks** (`~/.hello-io/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.hello-io/hooks/my-hook
cd ~/.hello-io/hooks/my-hook
```
...
metadata: { "hello-io": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
hello-io hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.hello-io/workspace`)
...
```yaml
metadata: { "hello-io": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "hello-io": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.hello-io/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.hello-io/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.hello-io/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/hello-io/hello-io/tree/main/src/hooks/bundled)
