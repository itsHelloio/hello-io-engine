...
```bash
hello-io tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `hello-io tui --deliver`
...
- Local shell commands receive `HELLO_IO_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `HELLO_IO_THEME=light` before launching `hello-io tui`.
- To force the original dark palette instead, set `HELLO_IO_THEME=dark`.
...
- Check the Gateway logs: `hello-io logs --follow`.
- Confirm the agent can run: `hello-io status` and `hello-io models status`.
...
- No agents in picker: check `hello-io agents list` and your routing config.
