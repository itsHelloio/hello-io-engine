...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hello-io`)
...
If the page fails to load, start the Gateway first: `hello-io gateway`.
...
you revoke it with `hello-io devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.hello-io/hello-io.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop hello-io`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. HelloIo
...
HelloIo **blocks** Control UI connections without device identity.
