...
read_when:
  - Running HelloIo Gateway in WSL2 while Chrome lives on Windows
...
- HelloIo Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/hello-io/hello-io/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the HelloIo Chrome extension.
...
- HelloIo points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, HelloIo is not the problem yet.
...
Fix that before touching HelloIo config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point HelloIo at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting HelloIo to succeed
...
```bash
hello-io browser open https://example.com --browser-profile remote
hello-io browser tabs --browser-profile remote
```

For the extension relay:

```bash
hello-io browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `hello-io browser tabs` returns the target
...
3. HelloIo config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug HelloIo config or Control UI auth
