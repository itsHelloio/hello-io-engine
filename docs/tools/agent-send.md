---
summary: "Direct `hello-io agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `hello-io agent` (direct agent runs)

`hello-io agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `hello-io message --target`).
...
```bash
hello-io agent --to +15555550123 --message "status update"
...
hello-io agent --to +15555550123 --message "Summon reply" --deliver
