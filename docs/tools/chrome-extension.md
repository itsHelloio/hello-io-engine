---
summary: "Chrome extension: let HelloIo drive your existing Chrome tab"
read_when:
...
The HelloIo Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate hello-io-managed Chrome profile.
...
HelloIo then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the HelloIo release (npm package) as static files. There is no separate “build” step.

After upgrading HelloIo:

- Re-run `hello-io browser extension install` to refresh the installed files under your HelloIo state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

HelloIo ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `HELLO_IO_GATEWAY_TOKEN`)
...
- Open the tab you want HelloIo to control.
...
- `ON`: attached; HelloIo can drive that tab.
...
Debugging: `hello-io sandbox explain`
...
`hello-io browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `hello-io browser extension install` first to copy the extension to a stable location under your HelloIo state directory.
...
- **This is not isolated** like the dedicated hello-io-managed profile.
