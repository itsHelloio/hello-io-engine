...
- **gateway host** → `hello-io` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.hello-io/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.hello-io/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `hello-io approvals allowlist ...`).
- `hello-io security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `hello-io doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.hello-io/exec-approvals.json` directly.

CLI: `hello-io approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
