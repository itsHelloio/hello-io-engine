...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hello-io sandbox explain`.
...
Legacy `agent.*` configs are migrated by `hello-io doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=hello-io-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${HELLO_IO_STATE_DIR:-$HOME/.hello-io}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
