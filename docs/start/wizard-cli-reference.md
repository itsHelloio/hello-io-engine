...
read_when:
  - You need detailed behavior for hello-io onboard
...
This page is the full reference for `hello-io onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.hello-io/hello-io.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `hello-io doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `hello-io pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `hello-io health`.
    - `hello-io status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.hello-io/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.hello-io/agents/<agentId>/agent/auth-profiles.json`
...
`~/.hello-io/credentials/oauth.json` (or `$HELLO_IO_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`hello-io onboard`](/cli/onboard)
