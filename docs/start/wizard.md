...
The onboarding wizard is the **recommended** way to set up HelloIo on macOS,
...
`hello-io dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`hello-io configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.hello-io/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `hello-io doctor` first.
</Note>
...
Use `hello-io agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.hello-io/workspace-<agentId>`.
...
- CLI command reference: [`hello-io onboard`](/cli/onboard)
