...
summary: "Architecture of HelloIo's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in HelloIo
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how HelloIo integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

HelloIo uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, HelloIo directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in HelloIo's local TUI mode)                                             |
...
├── pi-tools.ts                    # createHelloIoCodingTools()
...
├── hello-io-tools.ts              # HelloIo-specific tools
...
Image injection is prompt-local: HelloIo loads image refs from the current prompt and
...
2. **Custom Replacements**: HelloIo replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **HelloIo Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures HelloIo's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, HelloIo CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
HelloIo wraps this with `guardSessionManager()` for tool result safety.
...
HelloIo maintains an auth profile store with multiple API keys per provider:
...
HelloIo loads custom pi extensions for specialized behavior:
...
HelloIo also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | HelloIo Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom HelloIo tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.hello-io/agents/<agentId>/sessions/` (or `$HELLO_IO_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `HELLO_IO_LIVE_TEST=1`)
