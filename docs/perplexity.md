...
HelloIo supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, HelloIo also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `hello-io configure --section web`. It stores the key in
`~/.hello-io/hello-io.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.hello-io/.env` (or your service environment). See [Env vars](/help/faq#how-does-hello-io-load-environment-variables).
