...
- Publish as `hello-io/plugin-sdk` (or export from core under `hello-io/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `HelloIoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HelloIoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HelloIoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HelloIoConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HelloIoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HelloIoConfig): string;
  };
};
```
...
- Introduce `hello-io/plugin-sdk`.
- Add `api.runtime` to `HelloIoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `hello-ioRuntime: ">=2026.2.0"`).
