...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.hello-io/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.hello-io/node.json` (node id + pairing token).
...
Path: `~/.hello-io/exec-approvals.json`
...
- Unix socket at `~/.hello-io/exec-approvals.sock` (0600).
